 ...
 */
export interface InlinePrintOptions {
    /**
     * Whether the installed kea's `MakeLogicType` accepts a fourth `Meta` type argument (kea >= 4).
     * On older kea the metadata is intersected instead: `MakeLogicType<V, A, P> & logicMeta`.
     */
    metaTypeArgument: boolean
}

export function printInlineLogicType(
    parsedLogic: ParsedLogic,
    { metaTypeArgument }: InlinePrintOptions = { metaTypeArgument: true },
): string {
    ...
    let metaIntersection: ts.TypeNode | undefined
    if (metaMembers.length > 0) {
        ...
        if (!metaTypeArgument) {
            // kea 3: no Meta type parameter. Intersecting the interface narrows the loose
            // `key` / `sharedListeners` / `__keaTypeGenInternal*` members MakeLogicType declares.
            metaIntersection = typeArguments.pop()
        }
    }

    const makeLogicType = factory.createTypeReferenceNode(factory.createIdentifier('MakeLogicType'), typeArguments)
    const typeAlias = factory.createTypeAliasDeclaration(
        [factory.createModifier(SyntaxKind.ExportKeyword)],
        factory.createIdentifier(parsedLogic.logicTypeName),
        undefined,
        metaIntersection ? factory.createIntersectionTypeNode([makeLogicType, metaIntersection]) : makeLogicType,
    )
...
export async function writeInlineLogicTypes(
    ...
    for (const parsedLogic of parsedLogics) {
        ...
        const printedBlock = printInlineLogicType(parsedLogic, {
            metaTypeArgument: makeLogicTypeAcceptsMeta(program),
        })
        const block = (
        ...
        // Annotate untyped selector combiner parameters, e.g. `(counter) => ...` -> `(counter: number) => ...`:
        // MakeLogicType types them as `any`, so without an annotation the inferred value types would
        // degrade on the next run. When the input is one of this logic's own values (`s.counter`) the
        // annotation indexes the generated interface — `counter: counterLogicValues['counter']` — so it
        // follows that block whenever it is regenerated, instead of freezing whatever the checker saw
        // through the logic's previous (possibly `any`-tainted) type on the pass that wrote it.
        for (const { parameter, typeNode, sourceValue } of parsedLogic.selectorParamAnnotations) {
            const text =
                sourceValue && isOwnValue(parsedLogic, sourceValue)
                    ? `${parsedLogic.logicName}Values['${sourceValue}']`
                    : nodeToString(typeNode)
            const insertPos = (parameter.questionToken ?? parameter.name).getEnd()
            edits.push({ start: insertPos, end: insertPos, text: `: ${text}` })
        }
    }
...
/** Is `name` one of the logic's own values (a reducer or selector), i.e. a member of the generated Values interface? */
function isOwnValue(parsedLogic: ParsedLogic, name: string): boolean {
    return (
        parsedLogic.selectors.some((selector) => selector.name === name) ||
        parsedLogic.reducers.some((reducer) => reducer.name === name)
    )
}

const makeLogicTypeArityCache = new WeakMap<ts.Program, boolean>()

/**
 * Does the kea this program resolves declare `MakeLogicType` with a fourth (`Meta`) type parameter?
 * kea 3.x stops at `<Values, Actions, LogicProps>`; passing a fourth argument there is a type error.
 */
export function makeLogicTypeAcceptsMeta(program: ts.Program): boolean {
    const cached = makeLogicTypeArityCache.get(program)
    if (cached !== undefined) {
        return cached
    }
    let result: boolean | undefined
    for (const sourceFile of program.getSourceFiles()) {
        if (!/[\\/]kea[\\/]/.test(sourceFile.fileName) || !sourceFile.isDeclarationFile) {
            continue
        }
        for (const statement of sourceFile.statements) {
            if (
                (ts.isInterfaceDeclaration(statement) || ts.isTypeAliasDeclaration(statement)) &&
                statement.name.text === 'MakeLogicType'
            ) {
                result = (statement.typeParameters?.length ?? 0) >= 4
                break
            }
        }
        if (result !== undefined) {
    ...
    // kea not in the program at all (tests, odd setups): assume the current shape
    const accepts = result ?? true
    makeLogicTypeArityCache.set(program, accepts)
    return accepts
}
