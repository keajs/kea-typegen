...
describe('mixed mode with inlinePaths', () => {
    ...
    test('connected actions from an inline (MakeLogicType) logic land in the consuming logic type', async () => {
        ...
        try {
            ...
            // a type the consumers never import themselves: the checker prints it at the consumer
            // as import("/abs/path/types").Item, which must come out as a regular type import
            fs.writeFileSync(
                path.join(rootDir, 'types.ts'),
                ['export interface Item {', '    id: number', '}', ''].join('\n'),
            )
            fs.writeFileSync(
                inlineLogicPath,
                [
                    ...
                    '    actions: () => ({',
                    '        setValue: (value: string) => ({ value }),',
                    '        setItem: (item: Partial<Item>) => ({ item }),',
                    '    }),',
                    '    reducers: () => ({',
                    "        value: ['' as string, { setValue: (_, { value }) => value }],",
                    '    }),',
            ...
            fs.writeFileSync(
                ...
                [
                    ...
                    '    connect: {',
                    "        actions: [inlinedLogic, ['setValue', 'setItem']],",
            ...
            // a payload type out of scope at the consumer is imported, never left as import("/abs/path")
            expect(inlineConsumer).not.toContain('import("')
            expect(inlineConsumer).not.toContain("import('")
            expect(inlineConsumer).toContain("import type { Item } from '../types'")
            expect(inlineConsumer).toContain('setItem: (item: Partial<Item>) =>')
            expect(consumerType).not.toContain('import("')
            expect(consumerType).toContain("import type { Item } from './types'")
