...
var (
	...
	importTypeQualifierPattern     = regexp.MustCompile(`import\([^)]*\)\.`)
	importTypeReferencePattern     = regexp.MustCompile(`import\((?:'([^']+)'|"([^"]+)")\)\.([A-Z][A-Za-z0-9_$]*)`)
	namespaceTypeQualifierPattern  = regexp.MustCompile(`\b(?:[A-Za-z_$][A-Za-z0-9_$]*\.)+([A-Z][A-Za-z0-9_$]*)\b`)
)
...
type buildState struct {
	...
	typeTextBySymbolPos           map[string]string
	typeTextByLocation            map[string]string
	callbackByPos                 map[string]string
	signatureByPos                map[string]string
	exportedTypeNamesByFile       map[string]map[string]bool
	packageExportCandidatesByPath map[string]map[string]importCandidate
	importedValueSourcesByFile    map[string][]importedSourceFile
	transitiveTypeImportsByRef    map[string]cachedTypeImportsResult
...
func buildParsedLogic(report *Report, source string, sourceLogic SourceLogic, logicReport LogicReport, state *buildState) (ParsedLogic, error) {
	...
	defer maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "total", logicStart)
	...
	for index, section := range sections["actions"] {
		actions, actionImports := parseActionsWithSourceAndImports(section, source, sourcePropertyAt(properties["actions"], index), logicReport.InputKind, report.File, state)
		if source != "" && index >= len(properties["actions"]) {
			parsed.Actions = mergeParsedActionsPreferExisting(parsed.Actions, actions...)
			parsed.Imports = mergeTypeImports(parsed.Imports, actionImports)
			continue
		}
		parsed.Actions = mergeParsedActions(parsed.Actions, actions...)
		parsed.Imports = mergeTypeImports(parsed.Imports, actionImports)
	}
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "actions", actionsStart)
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "defaults", defaultsStart)
	reducersStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "reducers", reducersStart)
	formsStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "forms", formsStart)
	loadersStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "loaders", loadersStart)
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "connect", connectStart)
	selectorsStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "selectors", selectorsStart)
	sharedListenersStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "sharedListeners", sharedListenersStart)
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "events", eventsStart)
	internalReducerActionsStart := time.Now()
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "internalReducerActions", internalReducerActionsStart)
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "listeners", listenersStart)
	...
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "reorder", reorderStart)

	importsStart := time.Now()
	parsed.Imports = mergeTypeImports(parsed.Imports, collectTypeImports(source, report.File, parsed, state))
	parsed.Imports = preferSourceTypeImportsForUsedReferences(source, collectUsedTypeTexts(parsed), parsed.Imports)
	parsed.Imports = dropRedundantBarePackageExplicitImportTypeImports(parsed.Imports, collectUsedTypeTexts(parsed))
	maybeLogBuildTiming(filepath.Base(report.File), logicReport.Name, "imports", importsStart)
	return parsed, nil
}
...
func buildTimingsEnabled() bool {
	return os.Getenv("KEA_TYPEGEN_DEBUG_TIMINGS") == "1"
}

func maybeLogBuildTiming(fileName, logicName, phase string, started time.Time) {
	if !buildTimingsEnabled() || started.IsZero() {
		return
	}
	...
	if logicName != "" {
		fmt.Fprintf(os.Stderr, "[kea-typegen timing] %s :: %s :: %s = %s\n", fileName, logicName, phase, elapsed.Round(time.Millisecond))
		return
	}
	fmt.Fprintf(os.Stderr, "[kea-typegen timing] %s :: %s = %s\n", fileName, phase, elapsed.Round(time.Millisecond))
}
...
func parseActionsWithSource(
	...
	actions, _ := parseActionsWithSourceAndImports(section, source, property, inputKind, file, state)
	return actions
}

func parseActionsWithSourceAndImports(
	section SectionReport,
	source string,
	property SourceProperty,
	inputKind string,
	...
	for _, name := range orderedNames {
		...
		if ok {
			...
			imports = mergeTypeImports(imports, sourceIndexedAccessActionPayloadOverrideImports(source, file, sourceActionExpressionText(source, nested, entryExpression), state))
		}
	}
...
func parseSingleActionWithSource(
	...
	expression := sourceActionExpressionText(source, property, entryExpression)
		...
		if parameters, explicitReturn, ok := parseSourceArrowSignature(expression); ok {
			...
			if explicitReturn == "" {
				if inferred := sourceActionPayloadTypeFromSource(source, file, expression, state); inferred != "" {
					payloadType = mergePreferredActionPayloadTypePreservingCurrentMembers(
						payloadType,
						inferred,
						parityModeReportedAnyShorthandPayloadMembers(source, expression, parameters, payloadType),
					)
				}
			...
			if member != nil {
				if memberParameters, _, ok := splitFunctionType(functionType); ok && !sourceActionParametersArePreferred(parameters) {
					parameters = mergeActionParametersPreservingExplicitSource(parameters, memberParameters)
				}
			}
			functionType = parameters + " => " + payloadType
			functionType = preserveUntypedDestructuredActionParameters(functionType, sourceParameters)
		}
	}

	payloadType = normalizeActionPayloadType(payloadType)
	if expanded := specializeDefaultedGenericReferencesInTypeText(source, file, payloadType, state); expanded != "" {
		payloadType = expanded
	}
	...
	payloadType = refineActionPayloadTypeFromFunction(functionType, payloadType)
	functionType = refineActionFunctionReturnType(functionType, payloadType)
...
func sourceActionExpressionText(source string, property SourceProperty, entryExpression string) string {
	if property.ValueEnd > property.ValueStart {
		return sourcePropertyText(source, property)
	}
...
func sourceIndexedAccessActionPayloadOverrides(source, file, expression, parameters string, state *buildState) map[string]string {
	...
	for _, parameter := range parsedParameters {
		...
		if name, ok := sourceParameterName(parameter.Text); ok && name != "" {
		...
		if destructuredText := sourceParameterDestructuringText(parameter.Text); destructuredText != "" {
			originalMembers, ok := parseObjectTypeMembersWithOptionalUndefined(parameter.Type, true)
			if !ok || len(originalMembers) == 0 {
				continue
			}
			expandedHints := sourceDestructuredParameterTypeHints(source, file, destructuredText, parameter.Type, state)
			for name, sourceName := range sourceDestructuredParameterBindings(destructuredText) {
				originalType := normalizeSourceTypeText(strings.TrimSpace(originalMembers[sourceName]))
				if !strings.Contains(originalType, "[") {
					continue
				}
				typeText := normalizeSourceTypeText(expandedHints[name])
				typeText = normalizeSourceTypeText(typeText)
				if typeText == "" || !isUsableIndexedAccessActionPayloadOverrideType(typeText) {
					continue
				}
				indexedAccessParameterTypes[name] = typeText
			}
		}
	}
...
func sourceIndexedAccessActionPayloadOverrideImports(source, file, expression string, state *buildState) []TypeImport {
	if state == nil || file == "" {
		return nil
	}
	info, ok := parseSourceArrowInfo(expression)
	...
	parsedParameters, ok := parseFunctionParameters(info.Parameters)
	if !ok || len(parsedParameters) == 0 {
		return nil
	}
	...
	for _, parameter := range parsedParameters {
		...
		expanded := normalizeSourceTypeText(expandSourceParameterHintTypeText(source, file, parameter.Type, state))
		if expanded == "" || !isUsableIndexedAccessActionPayloadOverrideType(expanded) || seenExpanded[expanded] {
			continue
		}
		seenExpanded[expanded] = true
		if direct := collectTypeImportsForTypeTexts(source, file, []string{expanded}, state, nil); len(direct) > 0 {
			imports = mergeTypeImports(imports, direct)
			continue
		}
		candidate, ok := resolveIndexedAccessExpandedImportCandidate(source, file, parameter.Type, expanded, state, exportCache, packageExportCache)
		if !ok {
			continue
		}
		imports = mergeTypeImports(imports, []TypeImport{{
			Path:  candidate.Path,
...
func resolveIndexedAccessExpandedImportCandidate(
	source string,
	file string,
	originalType string,
	expanded string,
	...
	baseReference := indexedAccessBaseImportReference(originalType)
	if baseReference == "" {
	...
	candidate, ok := importsByAlias[baseReference]
	if !ok || candidate.Path == "" {
		return importCandidate{}, false
	}
	return resolveImportedExportCandidate(file, []string{candidate.Path}, expanded, state, exportCache, packageExportCache)
}

func indexedAccessBaseImportReference(typeText string) string {
	text := strings.TrimSpace(typeText)
	bracketIndex := strings.Index(text, "[")
	if bracketIndex == -1 {
		return ""
	}
	base := strings.TrimSpace(text[:bracketIndex])
	if dotIndex := strings.Index(base, "."); dotIndex != -1 {
		base = strings.TrimSpace(base[:dotIndex])
	}
	if base == "" || !isIdentifierStart(base[0]) {
		return ""
	}
	for index := 1; index < len(base); index++ {
		if !isIdentifierPart(base[index]) {
			return ""
		}
	}
	return base
...
func parityModeReportedAnyShorthandPayloadMembers(source, expression, parameters, currentPayloadType string) map[string]bool {
	...
	for _, segment := range segments {
		...
		if !actionShorthandPayloadShouldPreserveReportedAny(source, parameterType) {
			continue
		}
...
func actionShorthandPayloadShouldPreserveReportedAny(source, typeText string) bool {
	typeText = normalizeSourceTypeText(strings.TrimSpace(typeText))
	if typeText == "" || !strings.Contains(typeText, "|") {
		return false
	}
	if len(collectTypeImportsForTypeTexts(source, "", []string{typeText}, nil, nil)) > 0 {
		return false
	}
...
func mergeActionParametersPreservingExplicitSource(sourceParameters, memberParameters string) string {
	sourceParts, sourceOK := splitFunctionParameterParts(sourceParameters)
	memberParts, memberOK := splitFunctionParameterParts(memberParameters)
	if !sourceOK || !memberOK || len(sourceParts) != len(memberParts) {
		return memberParameters
	}

	merged := make([]string, 0, len(sourceParts))
	changed := false
	for index, sourcePart := range sourceParts {
		sourcePart = normalizeParameterDeclarationText(sourcePart)
		memberPart := normalizeParameterDeclarationText(memberParts[index])
		if sourcePart == "" {
			merged = append(merged, memberPart)
			if memberPart != "" {
				changed = true
			}
			continue
		}
		if parameterHasExplicitType(sourcePart) {
			merged = append(merged, sourcePart)
			if sourcePart != memberPart {
				changed = true
			}
			continue
		}
...
func parameterHasExplicitType(part string) bool {
	_, typeText, ok := splitTopLevelPropertyRaw(strings.TrimSpace(part))
	return ok && strings.TrimSpace(typeText) != ""
}
...
func preferredMemberFunctionTypeTextPreservingOptionalUndefined(member MemberReport) string {
	return normalizeSourceTypeTextWithOptions(strings.TrimSpace(preferredMemberTypeText(member)), false)
}
...
func preferredSelectorInputTupleTypeText(member MemberReport) string {
	if text := preferredPrintedOrRawTypeText(member.SelectorInputPrintedReturnTypeNode, member.SelectorInputReturnTypeString); strings.TrimSpace(text) != "" {
		return text
	}
	for _, text := range []string{
		...
		if tupleText := selectorInputTupleTypeTextFromSelectorMember(text); tupleText != "" {
			return tupleText
		}
	}
	return ""
...
func preferredSelectorProjectorReturnTypeText(member MemberReport) string {
	contextualSurface := strings.TrimSpace(preferredSelectorProjectorTypeText(member))
	directSurface := strings.TrimSpace(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString))
	contextualReturn := selectorProjectorReturnTypeText(member.ProjectorPrintedReturnTypeNode, member.ProjectorReturnTypeString, contextualSurface)
	directReturn := selectorProjectorReturnTypeText(member.ProjectorDirectPrintedReturnTypeNode, member.ProjectorDirectReturnTypeString, directSurface)
	memberReturn := strings.TrimSpace(preferredMemberReturnTypeText(member))

	if preferDirectProjectorReturnType(memberReturn, directReturn, contextualReturn) {
		return directReturn
	}
	if contextualReturn != "" {
...
func selectorProjectorReturnTypeText(printedReturn, rawReturn, surface string) string {
	if text := strings.TrimSpace(preferredPrintedOrRawTypeText(printedReturn, rawReturn)); text != "" {
		return normalizeSourceTypeTextWithOptions(text, false)
	}
	if returnType := selectorReturnTypeFromFunctionSurface(strings.TrimSpace(surface)); returnType != "" {
		return returnType
	}
	return ""
}

func preferDirectProjectorReturnType(memberReturn, directReturn, contextualReturn string) bool {
	memberReturn = normalizeInferredTypeText(strings.TrimSpace(memberReturn))
	directReturn = normalizeInferredTypeText(strings.TrimSpace(directReturn))
	contextualReturn = normalizeInferredTypeText(strings.TrimSpace(contextualReturn))

	switch {
	case directReturn == "":
		return false
	case contextualReturn == "":
		return true
	case directReturn == contextualReturn:
		return false
	case isAnyLikeType(directReturn) || isLooselyTypedType(directReturn):
		return true
	case parityModeReportedStructuredTypeShouldStayLoose(directReturn, contextualReturn):
		return true
	case memberReturn != "" && memberReturn == directReturn:
		if isAnyLikeType(contextualReturn) || isLooselyTypedType(contextualReturn) {
			return true
		}
		if booleanLikeSelectorType(contextualReturn) && !booleanLikeSelectorType(directReturn) {
			return true
		}
		if isBroadPrimitiveSelectorType(contextualReturn) && !isBroadPrimitiveSelectorType(directReturn) {
			return true
		}
	}
	return false
...
func preferredPrintedOrRawTypeText(printed, raw string) string {
	...
	if printed != "" && raw != "" && printedTypeAddsOnlyQualification(printed, raw) {
		return raw
	}
...
func printedTypeAddsOnlyQualification(printed, raw string) bool {
	originalPrinted := strings.TrimSpace(printed)
	originalRaw := strings.TrimSpace(raw)
	printed = normalizeSourceTypeTextWithOptions(originalPrinted, false)
	raw = normalizeSourceTypeTextWithOptions(originalRaw, false)
	if printed == "" || raw == "" {
		return false
	}
	if originalPrinted != originalRaw && printed == raw {
		return true
	}
	return qualificationStrippedTypeText(printed) == raw
}

func qualificationStrippedTypeText(text string) string {
	text = normalizeSourceTypeTextWithOptions(strings.TrimSpace(text), false)
	if text == "" {
		return ""
	}
	text = importTypeQualifierPattern.ReplaceAllString(text, "")
	text = namespaceTypeQualifierPattern.ReplaceAllString(text, "${1}")
	return normalizeSourceTypeTextWithOptions(text, false)
}
...
func sourceObjectLiteralTypeText(source, expression string) string {
	...
	for _, segment := range segments {
		...
		if strings.HasPrefix(text, "...") {
			...
			spreadMembers, spreadOptionals, ok := parseObjectTypeMembersWithOptionalFlags(spreadType, true)
			if !ok {
				continue
			}
			for name, value := range spreadMembers {
				properties[name] = value
				optionalProperties[name] = spreadOptionals[name]
			}
			continue
		}
		...
		if !ok {
			if shorthand, shorthandOK := sourceIdentifierExpression(text); shorthandOK {
				...
				if valueType != "" {
					...
					optionalProperties[shorthand] = false
				}
			}
			continue
		}
...
func parseSelectorsWithSource(
	...
	for _, name := range orderedNames {
		...
		if hasMember {
			...
			} else if selectorTypeIsBareLiteral(preferredMemberReturnType) && selectorMemberHasLooseReportedSurface(member) {
		...
		keepParityReportedSelectorType := hasMember && parityModeKeepsLooseReportedSelectorType(logic, source, file, seenNames, selectorExpression, reportedSelectorType, member, state)
		...
		looseParitySelectorType := ""
		if keepParityReportedSelectorType {
			looseParitySelectorType = parityModeLooseReportedSelectorType(contextLogic, source, file, selectorExpression, reportedSelectorType, member, state)
		}
		...
		if selectorType == "" && keepParityReportedSelectorType && looseParitySelectorType != "" {
			selectorType = looseParitySelectorType
		}
		if selectorType == "" && keepParityReportedSelectorType && reportedSelectorType != "" {
			selectorType = reportedSelectorType
		}
		...
		if source != "" && selectorExpression != "" && allowConstructorRecovery && allowSourceRecovery {
			if nested, ok := sourceMembers[name]; ok {
				if !hasMember || !selectorMemberHasLooseReportedSurface(member) {
					if probed := normalizePublicRecoveredSelectorType(contextLogic, source, file, selectorExpression, reportedSelectorType, sourceSelectorReturnTypeFromTypeProbe(source, file, nested, state), state); probed != "" &&
						(isAnyLikeType(selectorType) || isLooselyTypedType(selectorType)) &&
						(isAnyLikeType(reportedSelectorType) || isLooselyTypedType(reportedSelectorType)) &&
						!isAnyLikeType(probed) &&
						!isLooselyTypedType(probed) &&
						recoveredSelectorTypeLooksStrong(probed) {
						selectorType = probed
					}
				}
			}
		...
		if selectorSourceHasExplicitAnyInputState(selectorExpression) &&
			(parityModeReportedStructuredTypeShouldStayLoose(reportedSelectorType, selectorType) ||
	...
	for _, name := range orderedNames {
		...
		if hasMember {
			if field, ok := findParsedField(contextLogic.Selectors, name); ok {
				keepParityReportedSelectorType = parityModeKeepsLooseReportedSelectorType(contextLogic, source, file, seenNames, selectorExpression, field.Type, member, state)
				...
				if !keepParityReportedSelectorType &&
					parityModeEnabled() &&
					selectorMemberHasLooseReportedSurface(member) &&
					selectorSourceHasExplicitAnyInputState(selectorExpression) &&
					!selectorSourceHasRecoverableTypedUnnamedDependency(selectorExpression) &&
		...
		if nested, ok := sourceMembers[name]; ok {
			...
			if field, ok := findParsedField(contextLogic.Selectors, name); ok {
				...
				if fieldType != "" &&
					(fallbackReturnType == "" ||
						((isAnyLikeType(fallbackReturnType) || isLooselyTypedType(fallbackReturnType)) &&
							!isAnyLikeType(fieldType) &&
							!isLooselyTypedType(fieldType))) {
			...
			if _, currentReturnType, ok := splitFunctionType(functionType); ok {
				...
				if currentReturnType != "" &&
					!((isAnyLikeType(currentReturnType) || isLooselyTypedType(currentReturnType)) &&
						fieldType != "" &&
						!isAnyLikeType(fieldType) &&
						!isLooselyTypedType(fieldType)) {
			...
			if recovered := sourceInternalSelectorFunctionTypeWithFallbackReturn(contextLogic, source, file, expression, fallbackReturnType, state); recovered != "" {
				...
				if shouldPreferRecoveredInternalHelperFunctionType(functionType, recovered) ||
					(!parityModeEnabled() && unnamedDependencyPlaceholderHelperShouldPreferRecovered(selectorExpression, functionType, recovered)) ||
					...
					parityModeConcreteLogicalFallbackHelperShouldPreferRecovered(source, file, selectorExpression, functionType, recovered, state) ||
					parityModeOpaqueArrayHelperShouldPreferRecovered(selectorExpression, functionType, recovered) ||
					parityModeExplicitlyTypedUnnamedDependencyHelperShouldPreferRecovered(selectorExpression, functionType, recovered) ||
					parityModeStructuredOpaqueHelperShouldPreferRecovered(selectorExpression, functionType, recovered) ||
			...
			if probed := sourceSelectorProjectorFunctionTypeFromTypeProbe(contextLogic, source, file, nested, state); probed != "" {
				...
				if shouldPreferRecoveredInternalHelperFunctionType(functionType, probed) ||
					(!parityModeEnabled() && unnamedDependencyPlaceholderHelperShouldPreferRecovered(selectorExpression, functionType, probed)) ||
					...
					parityModeConcreteLogicalFallbackHelperShouldPreferRecovered(source, file, selectorExpression, functionType, probed, state) ||
					parityModeOpaqueArrayHelperShouldPreferRecovered(selectorExpression, functionType, probed) ||
					parityModeExplicitlyTypedUnnamedDependencyHelperShouldPreferRecovered(selectorExpression, functionType, probed) ||
					parityModeStructuredOpaqueHelperShouldPreferRecovered(selectorExpression, functionType, probed) ||
		...
		if selectorSourceHasExplicitAnyInputState(selectorExpression) &&
			parityModeReportedStructuredHelperFunctionTypeShouldStayReported(reportedFunctionType, functionType) {
			functionType = reportedFunctionType
		}
		if field, ok := findParsedField(contextLogic.Selectors, name); ok {
			if refined := internalHelperFunctionTypeWithConcreteSelectorReturn(functionType, field.Type); refined != "" {
				functionType = refined
			}
		}
		if recoveredStructured := parityModeStructuredOpaqueRecoveredHelperFunctionType(contextLogic, source, file, selectorExpression, functionType, state); recoveredStructured != "" {
			functionType = recoveredStructured
		}
		if hasMember {
			if opaqueTupleFunctionType := parityModeOpaqueSelectorInputTupleHelperFunctionType(contextLogic, member, source, file, selectorExpression, functionType, state); opaqueTupleFunctionType != "" {
				functionType = opaqueTupleFunctionType
			}
		}
		if explicitAnyFunctionType := parityModeExplicitAnyInputHelperFunctionType(selectorExpression, functionType); explicitAnyFunctionType != "" {
			functionType = explicitAnyFunctionType
		}
		if hasMember {
			if qualifiedFunctionType := internalHelperFunctionTypeWithSelectorInputPackageImportQualifiers(member, functionType); qualifiedFunctionType != "" {
				functionType = qualifiedFunctionType
			}
		}
...
func internalHelperFunctionTypeWithConcreteSelectorReturn(functionType, selectorType string) string {
	parameters, returnType, ok := splitFunctionType(functionType)
	if !ok {
		return ""
	}
	selectorType = normalizeInferredTypeText(strings.TrimSpace(selectorType))
	if selectorType == "" || isAnyLikeType(selectorType) || isLooselyTypedType(selectorType) {
		return ""
	}
	returnType = normalizeInferredTypeText(strings.TrimSpace(returnType))
	if returnType != "" && !isAnyLikeType(returnType) && !isLooselyTypedType(returnType) &&
		!helperReturnTypeShouldYieldToConcreteSelectorType(selectorType, returnType) {
		return ""
	}
	return parameters + " => " + selectorType
}

func helperReturnTypeShouldYieldToConcreteSelectorType(selectorType, returnType string) bool {
	if !parityModeEnabled() {
		return false
	}
	selectorType = normalizeInferredTypeText(strings.TrimSpace(selectorType))
	returnType = normalizeInferredTypeText(strings.TrimSpace(returnType))
	if selectorType == "" || returnType == "" || selectorType == returnType {
		return false
	}
	if parityModeReportedStructuredTypeShouldBeatOpaqueCandidate(selectorType, returnType) {
		return true
	}
	return strings.HasPrefix(selectorType, "{") &&
		strings.HasPrefix(returnType, "{") &&
		(typeTextContainsStandaloneToken(returnType, "any") || typeTextContainsStandaloneToken(returnType, "unknown")) &&
		!typeTextContainsStandaloneToken(selectorType, "any") &&
		!typeTextContainsStandaloneToken(selectorType, "unknown")
}
	...
	for index, param := range params {
		...
		if index < len(dependencyTypes) {
			if dependencyType := normalizeInternalHelperSignatureParameterType(dependencyTypes[index], true); dependencyType != "" {
				dependencyName := ""
				isSelectorDependency := false
				if index < len(dependencyNames) {
					dependencyName = strings.TrimSpace(dependencyNames[index])
				}
				preferConcreteSelectorDependencyType := false
				if dependencyName != "" {
					if _, ok := findParsedField(logic.Selectors, dependencyName); ok {
						isSelectorDependency = true
					}
					if isSelectorDependency &&
						!isAnyLikeType(dependencyType) &&
						!isLooselyTypedType(dependencyType) {
						preferConcreteSelectorDependencyType = true
					}
				}
				preserveLooseParitySelectorDependency := parityModeEnabled() &&
					isSelectorDependency &&
					(isAnyLikeType(paramType) || isLooselyTypedType(paramType))
				if paramType == "" ||
					(!preserveLooseParitySelectorDependency &&
						(isAnyLikeType(paramType) ||
							isLooselyTypedType(paramType) ||
							(preferConcreteSelectorDependencyType && dependencyType != paramType))) {
...
func parityModeRecoveredInternalHelperFunctionType(expression, current, recovered string, keepReportedLoose bool) string {
	...
	if ok {
		recoveredParameters, recoveredReturnType, recoveredOK := splitFunctionType(recovered)
		if recoveredOK {
			currentReturnType = normalizeInferredTypeText(strings.TrimSpace(currentReturnType))
			recoveredReturnType = normalizeInferredTypeText(strings.TrimSpace(recoveredReturnType))
			if currentReturnType != "" &&
				recoveredReturnType != "" &&
				currentReturnType != recoveredReturnType &&
				!isAnyLikeType(currentReturnType) &&
				!isLooselyTypedType(currentReturnType) &&
				resolveReportedSelectorType(currentReturnType, recoveredReturnType) == currentReturnType {
	...
	recoveredReturnType = normalizeInferredTypeText(strings.TrimSpace(recoveredReturnType))
	if parityModeRecoveredInternalHelperKeepsConcreteNullableLookupReturn(expression, current, recovered) {
		return recovered
	}
	if parityModeRecoveredInternalHelperKeepsConcreteDirectFallbackReturn(expression, current, recovered) {
		return recovered
	}
	if parityModeRecoveredInternalHelperKeepsConcreteObjectArrayReturn(current, recovered) {
		return recovered
	}
	if coarseOpaqueReturn := coarsenOpaqueRecoveredType(recoveredReturnType); coarseOpaqueReturn != "" && coarseOpaqueReturn != "any" {
		return recoveredParameters + " => " + coarseOpaqueReturn
	}
...
func parityModeRecoveredInternalHelperKeepsConcreteNullableLookupReturn(expression, current, recovered string) bool {
	if !parityModeEnabled() {
		return false
	}

	currentParameters, currentReturnType, ok := splitFunctionType(current)
	if !ok {
		return false
	}
	recoveredParameters, recoveredReturnType, ok := splitFunctionType(recovered)
	...
	currentReturnType = normalizeInferredTypeText(strings.TrimSpace(currentReturnType))
	recoveredReturnType = normalizeInferredTypeText(strings.TrimSpace(recoveredReturnType))
	if (!isAnyLikeType(currentReturnType) && !isLooselyTypedType(currentReturnType)) ||
		recoveredReturnType == "" ||
		(isAnyLikeType(recoveredReturnType) || isLooselyTypedType(recoveredReturnType)) ||
		(!normalizedUnionContainsType(recoveredReturnType, "null") && !normalizedUnionContainsType(recoveredReturnType, "undefined")) {
		return false
	}
	...
	if !ok || len(recoveredParams) != len(currentParams) {
		return false
	}
	sawOpaqueCurrentParameter := false
	sawInformativeRecoveredParameter := false
	for index := range currentParams {
		currentType := normalizeInferredTypeText(strings.TrimSpace(currentParams[index].Type))
		if currentType == "" || isAnyLikeType(currentType) || isLooselyTypedType(currentType) {
			sawOpaqueCurrentParameter = true
		}
		recoveredType := normalizeInferredTypeText(strings.TrimSpace(recoveredParams[index].Type))
		if recoveredType != "" && !isAnyLikeType(recoveredType) && !isLooselyTypedType(recoveredType) {
			sawInformativeRecoveredParameter = true
		}
	}
	if !sawOpaqueCurrentParameter || !sawInformativeRecoveredParameter {
		return false
	}
	...
	left, ok := selectorNullableLookupExpression(body)
	return ok && selectorLookupCallLooksRecoverable(left)
}

func parityModeRecoveredInternalHelperKeepsConcreteDirectFallbackReturn(expression, current, recovered string) bool {
	if !parityModeEnabled() {
		return false
	}
	...
	if (!isAnyLikeType(currentReturnType) && !isLooselyTypedType(currentReturnType)) ||
		recoveredReturnType == "" ||
		isAnyLikeType(recoveredReturnType) ||
		isLooselyTypedType(recoveredReturnType) ||
		!recoveredSelectorTypeLooksStrong(recoveredReturnType) {
		return false
	}
	...
	sawOpaqueCurrentParameter := false
	sawInformativeRecoveredParameter := false
	dependencyTypes := make([]string, 0, len(recoveredParams))
	for index := range currentParams {
		currentType := normalizeInferredTypeText(strings.TrimSpace(currentParams[index].Type))
		if currentType == "" || isAnyLikeType(currentType) || isLooselyTypedType(currentType) {
			sawOpaqueCurrentParameter = true
		...
		dependencyTypes = append(dependencyTypes, recoveredType)
	}
	if !sawOpaqueCurrentParameter || !sawInformativeRecoveredParameter {
		return false
	}
	...
	return internalHelperDirectFallbackConcreteDependencyReturnType(expression, body, dependencyTypes) == recoveredReturnType
}

func parityModeRecoveredInternalHelperKeepsConcreteObjectArrayReturn(current, recovered string) bool {
	if !parityModeEnabled() {
		return false
	}
	...
	elementType, ok := parseNormalizedArrayElementType(recoveredReturnType)
	if !ok || elementType == "" {
		return false
	}
	elementType = normalizeInferredTypeText(strings.TrimSpace(elementType))
	if elementType == "" ||
		isAnyLikeType(elementType) ||
		isLooselyTypedType(elementType) ||
		isPrimitiveLikeUnionType(elementType) ||
		isBroadPrimitiveSelectorType(elementType) ||
		selectorTypeIsLiteralUnionOfKind(elementType, "string") ||
		selectorTypeIsLiteralUnionOfKind(elementType, "number") ||
		selectorTypeIsLiteralUnionOfKind(elementType, "boolean") {
		return false
	}
	...
	return sawOpaqueCurrentParameter && sawInformativeRecoveredParameter
}
...
func parityModeRecoveredInternalHelperParameterTypes(expression, current, recovered string) string {
	...
	for index, param := range recoveredParams {
		...
		} else if (isAnyLikeType(currentType) || isLooselyTypedType(currentType)) &&
			parityModeRecoveredInternalHelperKeepsLooseNamedPrimitiveParameter(paramType) {
			paramType = currentType
			if paramType != normalizeInferredTypeText(strings.TrimSpace(param.Type)) {
				changed = true
			}
...
func parityModeRecoveredInternalHelperKeepsLooseNamedPrimitiveParameter(typeText string) bool {
	typeText = normalizeInferredTypeText(strings.TrimSpace(typeText))
	if typeText == "" || isAnyLikeType(typeText) || isLooselyTypedType(typeText) {
		return false
	}
	return isBroadPrimitiveSelectorType(typeText) ||
...
func parityModeStructuredOpaqueHelperShouldPreferRecovered(expression, current, candidate string) bool {
	if !parityModeEnabled() || selectorSourceHasExplicitAnyInputState(expression) {
		return false
	}
	...
	if structuredTypeUsesOpaqueLoosePreservation(currentReturn, candidateReturn) {
		return true
	}
	return parityModeReportedStructuredTypeShouldStayLoose(currentReturn, candidateReturn) ||
		parityModeReportedStructuredTypeShouldBeatOpaqueCandidate(currentReturn, candidateReturn)
}

func parityModeOpaqueArrayHelperShouldPreferRecovered(expression, current, candidate string) bool {
	if !parityModeEnabled() || selectorSourceHasExplicitAnyInputState(expression) || !internalHelperComplexRecoveryShouldStayOpaque(current, candidate) {
		return false
	}
	_, candidateReturn, ok := splitFunctionType(candidate)
	if !ok {
	...
	candidateReturn = normalizeInferredTypeText(strings.TrimSpace(candidateReturn))
	return candidateReturn != "" && isArrayLikeType(candidateReturn)
}

func unnamedDependencyPlaceholderHelperShouldPreferRecovered(expression, current, candidate string) bool {
	currentParameters, currentReturn, ok := splitFunctionType(current)
	if !ok {
		return false
	}
	...
	placeholderNames := internalHelperParameterNames(dependencyNames, nil)
	if len(placeholderNames) != len(candidateParams) {
		return false
	}
	...
	for index := range candidateParams {
		...
		currentType = normalizeInternalHelperParameterType(currentType)
		candidateType = normalizeInternalHelperParameterType(candidateType)
		...
		if candidateName != placeholderNames[index] {
			return false
		}
...
func parityModeUnnamedDependencyPlaceholderHelperShouldPreferRecovered(expression, current, candidate string) bool {
	...
	for index := range candidateParams {
		...
		keepRecoveredPlaceholderName := index < len(dependencyExpressions) &&
			sourceSelectorUnnamedDependencyKeepsRecoveredType(dependencyExpressions[index])
		if !keepRecoveredPlaceholderName &&
			!(isAnyLikeType(currentType) || isLooselyTypedType(currentType)) {
			continue
		}
		sawPlaceholderRename = true
	}
...
func parityModeConcreteLogicalFallbackHelperShouldPreferRecovered(source, file, expression, current, candidate string, state *buildState) bool {
	if !parityModeEnabled() || !internalHelperComplexRecoveryShouldStayOpaque(current, candidate) {
		return false
	}

	body := selectorProjectorReturnBody(expression)
	if body == "" || (findLastTopLevelOperator(body, "??") == -1 && findLastTopLevelOperator(body, "||") == -1) {
		return false
	}
	...
	candidateReturn = normalizeInferredTypeText(strings.TrimSpace(candidateReturn))
	if candidateReturn == "" || isAnyLikeType(candidateReturn) || isLooselyTypedType(candidateReturn) {
		return false
	}

	candidateParams, ok := parseFunctionParameters(candidateParameters)
	if ok && len(candidateParams) > 0 {
		dependencyTypes := make([]string, 0, len(candidateParams))
		for _, param := range candidateParams {
			dependencyTypes = append(dependencyTypes, normalizeInferredTypeText(strings.TrimSpace(param.Type)))
		}
		if concreteReturn := internalHelperDirectFallbackConcreteDependencyReturnType(expression, body, dependencyTypes); concreteReturn != "" &&
			concreteReturn == candidateReturn {
			return true
		}
	}

	hints := sourceParameterTypeHintsWithContext(source, file, candidateParameters, state)
	if len(hints) == 0 {
		return false
	}
	inferred := normalizeInferredTypeText(sourceExpressionTypeTextWithContext(source, file, body, hints, state))
	return inferred != "" && inferred == candidateReturn
}
...
func selectorMemberHasLooseReportedSurface(member MemberReport) bool {
	if selectorMemberHasLooseReportedReturn(member.TypeString) {
		return true
	}
	for _, text := range []string{
		member.ReturnTypeString,
		member.ProjectorReturnTypeString,
		member.ProjectorDirectReturnTypeString,
	} {
		text = normalizeInferredTypeText(strings.TrimSpace(text))
		if isAnyLikeType(text) || isLooselyTypedType(text) {
			return true
		}
	}
	if directReturn := selectorReturnTypeFromFunctionSurface(strings.TrimSpace(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString))); directReturn != "" &&
		(isAnyLikeType(directReturn) || isLooselyTypedType(directReturn)) {
		return true
	}
	return false
}

func selectorMemberHasLooseReportedConflict(member MemberReport, reportedType string) bool {
	...
	contextualReturn := selectorProjectorReturnTypeText(
		member.ProjectorPrintedReturnTypeNode,
		member.ProjectorReturnTypeString,
		strings.TrimSpace(preferredSelectorProjectorTypeText(member)),
	)
	contextualReturn = normalizeInferredTypeText(strings.TrimSpace(contextualReturn))
	if contextualReturn == "" || isAnyLikeType(contextualReturn) || isLooselyTypedType(contextualReturn) {
		return false
	}
	return contextualReturn != reportedType
}
...
func parityModeKeepsLooseReportedSelectorType(
	...
	if !selectorMemberHasLooseReportedSurface(member) {
		return false
	}
	if selectorMemberHasLooseReportedConflict(member, reportedType) {
		return true
	}
	if selectorSourceHasRecoverableTypedUnnamedDependency(expression) {
		return false
	}
	inferredType := sourceSelectorInferredType(logic, source, file, expression, state)
	if parityModeKeepsOpaqueReportedPrimitiveArraySelectorType(expression, reportedType, member, inferredType) {
		return true
	}
	if parityModeKeepsOpaqueReportedNullableLookupWithConcreteProjectorParams(expression, reportedType, member, inferredType) {
		return true
	}
	dependencyTypes := sourceSelectorDependencyTypes(logic, source, file, expression, state)
	if opaqueIndexedFallbackRecoveryShouldStayConcrete(
		expression,
		selectorProjectorReturnBody(expression),
		inferredType,
		dependencyTypes,
	) {
		return false
	}
	if selectorOpaqueNullableLookupRecoveryShouldStayConcrete(logic, source, file, expression, inferredType, state) {
		return false
	}
	if selectorDependencyTypesAreConcrete(sourceSelectorDependencyTypesWithPlaceholders(logic, source, file, expression, state)) {
		return false
	}
	knownFields := mergeParsedFields(logic.Reducers, logic.Selectors...)
...
func parityModeKeepsOpaqueReportedPrimitiveArraySelectorType(expression, reportedType string, member MemberReport, inferredType string) bool {
	...
	inferredType = normalizeInferredTypeText(strings.TrimSpace(inferredType))
	elementType, ok := parseNormalizedArrayElementType(inferredType)
	if !ok {
		return false
	}
	elementType = normalizeInferredTypeText(strings.TrimSpace(elementType))
	if !(isPrimitiveLikeUnionType(elementType) ||
		isBroadPrimitiveSelectorType(elementType) ||
		selectorTypeIsLiteralUnionOfKind(elementType, "string") ||
		selectorTypeIsLiteralUnionOfKind(elementType, "number") ||
		selectorTypeIsLiteralUnionOfKind(elementType, "boolean")) {
	...
	return selectorMemberHasLooseReportedSurface(member) &&
		isAnyLikeType(normalizeInferredTypeText(strings.TrimSpace(selectorProjectorReturnTypeText(
			member.ProjectorDirectPrintedReturnTypeNode,
			member.ProjectorDirectReturnTypeString,
			strings.TrimSpace(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString)),
		))))
}

func parityModeLooseReportedSelectorType(logic ParsedLogic, source, file, expression, reportedType string, member MemberReport, state *buildState) string {
	if !parityModeEnabled() || strings.TrimSpace(expression) == "" || sourceSelectorReturnType(expression) != "" {
		return ""
	}
	inferredType := sourceSelectorInferredType(logic, source, file, expression, state)
	if loosePrimitiveArray := parityModeLooseReportedPrimitiveArraySelectorType(expression, reportedType, member, inferredType); loosePrimitiveArray != "" {
		return loosePrimitiveArray
	}
	return ""
}

func parityModeLooseReportedPrimitiveArraySelectorType(expression, reportedType string, member MemberReport, inferredType string) string {
	if !parityModeKeepsOpaqueReportedPrimitiveArraySelectorType(expression, reportedType, member, inferredType) {
		return ""
	}
	return coarsenOpaqueRecoveredType(inferredType)
}

func parityModeKeepsOpaqueReportedNullableLookupWithConcreteProjectorParams(expression, reportedType string, member MemberReport, inferredType string) bool {
	if !parityModeEnabled() || strings.TrimSpace(expression) == "" || sourceSelectorReturnType(expression) != "" {
		return false
	}
	reportedType = normalizeInferredTypeText(strings.TrimSpace(reportedType))
	...
	inferredType = normalizeInferredTypeText(strings.TrimSpace(inferredType))
	if inferredType == "" ||
		(!strings.Contains(inferredType, "| null") &&
			!strings.Contains(inferredType, "| undefined") &&
			!strings.Contains(inferredType, "null |") &&
			!strings.Contains(inferredType, "undefined |")) ||
		!recoveredSelectorTypeLooksStrong(inferredType) {
		return false
	}
	body := selectorProjectorReturnBody(expression)
	if body == "" {
	...
	directType := strings.TrimSpace(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString))
	parameters, directReturn, ok := splitFunctionType(directType)
	if !ok {
		return false
	}
	directReturn = normalizeInferredTypeText(strings.TrimSpace(directReturn))
	if !isAnyLikeType(directReturn) && !isLooselyTypedType(directReturn) {
		return false
	}
	params, ok := parseFunctionParameters(parameters)
	if !ok || len(params) == 0 {
...
func selectorDependencyTypesAreConcrete(dependencyTypes []string) bool {
...
func normalizePublicRecoveredSelectorType(logic ParsedLogic, source, file, expression, reportedType, candidate string, state *buildState) string {
	...
	if coerced := parityModeCoarsenedOpaqueSelectorRecovery(logic, source, file, expression, reportedType, normalized, state); coerced != "" {
		return coerced
	}
	...
	if selectorSourceHasExplicitAnyInputState(expression) &&
		(parityModeReportedStructuredTypeShouldStayLoose(reportedType, normalized) ||
...
func parityModeCoarsenedOpaqueSelectorRecovery(logic ParsedLogic, source, file, expression, reportedType, typeText string, state *buildState) string {
	if !parityModeEnabled() {
		return ""
	}
	reportedType = normalizeInferredTypeText(strings.TrimSpace(reportedType))
	...
	if !recoveredSelectorTypeLooksStrong(typeText) || !selectorDependencyTypesLookConcrete(logic, source, file, expression, state) {
		return ""
	}
	...
	return coarsenOpaqueRecoveredType(typeText)
}

func coarsenOpaqueRecoveredType(typeText string) string {
	text := normalizeInferredTypeText(strings.TrimSpace(typeText))
	switch {
	case text == "":
		return ""
	case strings.HasPrefix(text, "readonly ") && strings.HasSuffix(text, "[]"):
		return "readonly any[]"
	case strings.HasPrefix(text, "ReadonlyArray<") && strings.HasSuffix(text, ">"):
...
func selectorOpaqueComplexRecoveryShouldStayAny(logic ParsedLogic, source, file, expression, typeText string, state *buildState) bool {
	...
	if body != "" {
		dependencyTypes := sourceSelectorDependencyTypes(logic, source, file, expression, state)
		if internalHelperDirectFallbackConcreteDependencyReturnType(expression, body, dependencyTypes) == typeText {
			return false
		}
	}
	return !selectorDependencyTypesLookConcrete(logic, source, file, expression, state)
...
func selectorOpaqueNullableLookupRecoveryShouldStayConcrete(
	...
	return sawConcreteNonPrimitiveDependency &&
		(sawOpaqueDependency || selectorDependencyTypesLookConcrete(logic, source, file, expression, state))
}
...
func selectorLookupCallLooksRecoverable(expression string) bool {
	...
	if _, path, ok := parseSourceAccessPath(text); ok {
...
func selectorNullableLookupExpression(expression string) (string, bool) {
	if left, ok := selectorSimpleFallbackAccessExpression(expression); ok {
		return left, true
	}
	...
	trueExpression := strings.TrimSpace(text[questionIndex+1 : colonIndex])
	falseExpression := strings.TrimSpace(text[colonIndex+1:])
	if trueExpression == "" || falseExpression == "" {
		return "", false
	}
	if !sourceFallbackLiteralShouldDefer(sourceLiteralTypeText(falseExpression)) {
		return "", false
	}
	return trueExpression, true
}
...
func shouldPreferInternalHelperSelectorReturn(current, helperFunctionType, helperReturn string, allowOpaqueCurrentRefinement bool) bool {
	...
	if isAnyLikeType(current) || isLooselyTypedType(current) {
		return internalHelperReturnCanRefineOpaqueSelector(helperFunctionType, helperReturn) ||
			internalHelperConcreteNullableReturnCanRefineOpaqueSelector(helperFunctionType, helperReturn)
	}
	...
	if parityModeEnabled() &&
		(structuredTypeUsesOpaqueLoosePreservation(current, helperReturn) ||
			parityModeReportedStructuredTypeShouldStayLoose(current, helperReturn) ||
			parityModeReportedStructuredTypeShouldBeatOpaqueCandidate(current, helperReturn)) {
		return true
	}
...
func internalHelperConcreteNullableReturnCanRefineOpaqueSelector(functionType, returnType string) bool {
	if !parityModeEnabled() {
		return false
	}
	...
	if text == "" ||
		isAnyLikeType(text) ||
		isLooselyTypedType(text) ||
		(!normalizedUnionContainsType(text, "null") && !normalizedUnionContainsType(text, "undefined")) {
		return false
	}
	if !recoveredSelectorTypeLooksStrong(text) &&
		!isPrimitiveLikeUnionType(text) &&
		!isBroadPrimitiveSelectorType(text) {
		return false
	}
	...
	sawInformativeParameter := false
	sawOpaqueParameter := false
	for _, param := range params {
		paramType := normalizeInferredTypeText(strings.TrimSpace(param.Type))
		if paramType == "" || isAnyLikeType(paramType) || isLooselyTypedType(paramType) {
			sawOpaqueParameter = true
			continue
		}
		sawInformativeParameter = true
	}
	return sawInformativeParameter && sawOpaqueParameter
}
...
func selectorMemberTypeTextSupportsInternalHelper(text string) bool {
	...
	return selectorFunctionTypeFromSurface(text) != ""
}
...
func selectorFunctionTypeFromMember(member MemberReport) string {
	contextualFunctionType := selectorFunctionTypeFromSurface(strings.TrimSpace(preferredSelectorProjectorTypeText(member)))
	directFunctionType := selectorFunctionTypeFromSurface(strings.TrimSpace(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString)))
	if preferDirectProjectorReturnType(
		preferredMemberReturnTypeText(member),
		selectorFunctionReturnTypeText(directFunctionType),
		selectorFunctionReturnTypeText(contextualFunctionType),
	) {
		return directFunctionType
	}
	if contextualFunctionType != "" {
		return contextualFunctionType
	}
	...
	for _, text := range []string{
		...
		if functionType := selectorFunctionTypeFromSurface(text); functionType != "" {
			return functionType
		}
	}
	return ""
...
func selectorFunctionReturnTypeText(functionType string) string {
	_, returnType, ok := splitFunctionType(functionType)
	if !ok {
		return ""
	}
...
func selectorReturnTypeFromFunctionSurface(text string) string {
	functionType := selectorFunctionTypeFromSurface(text)
	if functionType == "" {
		return ""
	}
	_, returnType, ok := splitFunctionType(functionType)
	...
	return normalizeSourceTypeTextWithOptions(strings.TrimSpace(returnType), false)
}

func selectorFunctionTypeFromSurface(text string) string {
	for _, candidate := range selectorFunctionTypeSurfaceCandidates(text) {
		parameters, returnType, ok := splitFunctionType(candidate)
		if !ok || strings.TrimSpace(firstParameterText(parameters)) == "" {
			continue
		}
...
func selectorFunctionTypeSurfaceCandidates(text string) []string {
	seen := map[string]bool{}
	...
	add := func(candidate string) {
		...
		unwrapped := normalizeSourceTypeTextWithOptions(strings.TrimSpace(unwrapWrappedExpression(candidate)), false)
		if unwrapped != "" && !seen[unwrapped] {
			seen[unwrapped] = true
	...
	add(text)
	if element := collapsedFunctionArrayElementType(text); element != "" {
		add(element)
	}
	...
	if parts, err := splitTopLevelUnion(text); err == nil && len(parts) > 1 {
		for _, part := range parts {
			add(part)
		}
	}
...
func collapsedFunctionArrayElementType(text string) string {
	text = normalizeSourceTypeTextWithOptions(strings.TrimSpace(text), false)
	if text == "" || !strings.HasSuffix(text, "[]") {
		return ""
	}
	element := strings.TrimSpace(unwrapWrappedExpression(text[:len(text)-2]))
	if element == "" {
		return ""
	}
	return normalizeSourceTypeTextWithOptions(element, false)
}
...
func parityModeOpaqueSelectorInputTupleHelperFunctionType(logic ParsedLogic, member MemberReport, source, file, expression, current string, state *buildState) string {
	if !parityModeEnabled() {
		return ""
	}
	tupleText := strings.TrimSpace(preferredSelectorInputTupleTypeText(member))
	elementTypes := selectorInputTupleElementTypes(tupleText)
	currentParameters, currentReturnType, ok := splitFunctionType(current)
	if !ok {
		return ""
	}
	currentReturnType = normalizeSelectorOptionalUndefinedTypeText(strings.TrimSpace(currentReturnType))
	if currentReturnType == "" {
		return ""
	}
	currentParams, ok := parseFunctionParameters(currentParameters)
	...
	explicitAnyState := selectorSourceHasExplicitAnyInputState(expression)
	dependencyNames := sourceSelectorDependencyNamesWithPlaceholders(expression)
	if len(elementTypes) == 0 && isAnyArrayType(normalizeInferredTypeText(tupleText)) {
		count := len(dependencyNames)
		if count == 0 {
			count = len(currentParams)
		}
		if count > 0 {
			elementTypes = make([]string, count)
			for index := range elementTypes {
				elementTypes[index] = "any"
			}
		}
	}
	if len(elementTypes) == 0 || len(currentParams) != len(elementTypes) {
		return ""
	}
	sawOpaqueTupleElement := false
	for _, elementType := range elementTypes {
		if isAnyLikeSelectorHelperType(elementType) {
			sawOpaqueTupleElement = true
		}
	}
	if !sawOpaqueTupleElement {
		return ""
	}
	knownFields := mergeParsedFields(logic.Reducers, logic.Selectors...)
	dependencyTypes := sourceSelectorDependencyTypesWithPlaceholders(logic, source, file, expression, state)
	rebuilt := make([]string, 0, len(currentParams))
	changed := false
	for index, param := range currentParams {
		rawName, _, ok := splitTopLevelPropertyRaw(param.Text)
		if !ok {
			return ""
		}
		...
		paramType := normalizeInferredTypeText(strings.TrimSpace(param.Type))
		if index < len(elementTypes) && isAnyLikeSelectorHelperType(elementTypes[index]) {
			switch {
			case explicitAnyState:
				paramType = "any"
			case index < len(dependencyNames):
				dependencyName := strings.TrimSpace(dependencyNames[index])
				if dependencyName == "" {
					break
				}
				if field, ok := findParsedField(knownFields, dependencyName); ok {
					fieldType := normalizeInternalHelperSignatureParameterType(field.Type, true)
					if parityModeOpaqueTupleHelperKeepsResolvedDependencyType(fieldType) {
						paramType = fieldType
						break
					}
				}
				if index < len(dependencyTypes) {
					dependencyType := normalizeInternalHelperSignatureParameterType(dependencyTypes[index], true)
					if parityModeOpaqueTupleHelperKeepsResolvedDependencyType(dependencyType) {
						paramType = dependencyType
						break
					}
				}
		...
		rebuilt = append(rebuilt, fmt.Sprintf("%s: %s", name, paramType))
	}
	if !changed {
		return ""
	}
	return "(" + strings.Join(rebuilt, ", ") + ") => " + currentReturnType
}

func parityModeOpaqueTupleHelperKeepsResolvedDependencyType(typeText string) bool {
	typeText = normalizeInternalHelperSignatureParameterType(typeText, true)
	if typeText == "" {
		return false
	}
	return (!isAnyLikeType(typeText) && !isLooselyTypedType(typeText)) ||
		isAnyArrayType(typeText) ||
		typeText == "readonly any[]" ||
		typeText == "ReadonlyArray<any>"
}

func parityModeExplicitAnyInputHelperFunctionType(expression, current string) string {
	if !parityModeEnabled() || !selectorSourceHasExplicitAnyInputState(expression) {
		return ""
	}
	parameters, returnType, ok := splitFunctionType(current)
	if !ok {
		return ""
	}
	params, ok := parseFunctionParameters(parameters)
	...
	for index, param := range params {
		...
		if normalizeInferredTypeText(strings.TrimSpace(param.Type)) != "any" {
			changed = true
		}
		rebuilt = append(rebuilt, fmt.Sprintf("%s: any", name))
	}
	if !changed {
		return ""
	}
...
func internalHelperFunctionTypeWithSelectorInputPackageImportQualifiers(member MemberReport, current string) string {
	tupleText := strings.TrimSpace(member.SelectorInputPrintedReturnTypeNode)
	if tupleText == "" || !strings.Contains(tupleText, "import(") {
		return ""
	}
	elementTypes := selectorInputTupleElementTypesPreservingPackageImports(tupleText)
	if len(elementTypes) == 0 {
		return ""
	}
	...
	params, ok := parseFunctionParameters(parameters)
	if !ok || len(params) != len(elementTypes) {
		return ""
	}
	...
	for index, param := range params {
		...
		paramType := normalizeSourceTypeTextWithOptions(strings.TrimSpace(param.Type), false)
		candidate := normalizePackageImportQualifiedTypeText(elementTypes[index])
		if candidate != "" &&
			strings.Contains(candidate, "import(") &&
			normalizeSourceTypeTextWithOptions(candidate, false) == paramType {
			paramType = candidate
			changed = true
		}
		if paramType == "" {
...
func selectorInputTupleElementTypesPreservingPackageImports(text string) []string {
	...
	for _, part := range parts {
		...
		if _, returnType, ok := splitFunctionType(part); ok {
			elementTypes = append(elementTypes, normalizePackageImportQualifiedTypeText(returnType))
			continue
		}
		elementTypes = append(elementTypes, normalizePackageImportQualifiedTypeText(part))
	}
	return elementTypes
}

func normalizePackageImportQualifiedTypeText(typeText string) string {
	text := strings.TrimSpace(typeText)
	if text == "" {
		return ""
	}
	text = strings.Join(strings.Fields(text), " ")
	text = stripRelativeImportTypeQualifiers(text)
...
func stripRelativeImportTypeQualifiers(typeText string) string {
	matches := importTypeReferencePattern.FindAllStringSubmatchIndex(typeText, -1)
	if len(matches) == 0 {
		return typeText
	}
	...
	for _, match := range matches {
		moduleStart, moduleEnd := match[2], match[3]
		if moduleStart == -1 || moduleEnd == -1 {
			moduleStart, moduleEnd = match[4], match[5]
		}
		nameStart, nameEnd := match[6], match[7]
		if moduleStart == -1 || moduleEnd == -1 || nameStart == -1 || nameEnd == -1 {
			continue
		}
		modulePath := strings.TrimSpace(typeText[moduleStart:moduleEnd])
		if !strings.HasPrefix(modulePath, ".") && !strings.HasPrefix(modulePath, "/") {
			continue
		}
		builder.WriteString(typeText[last:match[0]])
		builder.WriteString(typeText[nameStart:nameEnd])
		last = match[1]
		changed = true
	}
	if !changed {
	...
	builder.WriteString(typeText[last:])
	return builder.String()
}

func parityModeStructuredOpaqueRecoveredHelperFunctionType(logic ParsedLogic, source, file, expression, current string, state *buildState) string {
	if !parityModeEnabled() || selectorSourceHasExplicitAnyInputState(expression) || strings.TrimSpace(expression) == "" {
		return ""
	}
	_, currentReturn, ok := splitFunctionType(current)
	if !ok {
	...
	recovered := sourceInternalSelectorFunctionTypeWithFallbackReturn(logic, source, file, expression, currentReturn, state)
	if recovered == "" || recovered == current {
		return ""
	}
	recovered = canonicalizeInternalHelperFunctionTypeWithSelectorDependencies(logic, source, file, expression, recovered, state)
	_, candidateReturn, ok := splitFunctionType(recovered)
	if !ok || !recoveredSelectorTypeLooksStrong(candidateReturn) {
		return ""
	}
	if structuredTypeUsesOpaqueLoosePreservation(currentReturn, candidateReturn) ||
		parityModeReportedStructuredTypeShouldStayLoose(currentReturn, candidateReturn) ||
		parityModeReportedStructuredTypeShouldBeatOpaqueCandidate(currentReturn, candidateReturn) {
		return recovered
	}
	return ""
...
func sourceInternalSelectorFunctionTypeWithFallbackReturn(logic ParsedLogic, source, file, expression, fallbackReturnType string, state *buildState) string {
	...
	if returnType == "" {
		...
		if fallbackReturnType != "" &&
			!isAnyLikeType(fallbackReturnType) &&
			!isLooselyTypedType(fallbackReturnType) &&
			(isAnyLikeType(inferredReturnType) || isLooselyTypedType(inferredReturnType)) {
			returnType = fallbackReturnType
		} else if internalHelperOpaqueComplexFallbackReturnShouldStayAny(logic, source, file, expression, info.Body, info.BlockBody, fallbackReturnType, inferredReturnType, dependencyTypes, state) ||
...
func internalHelperOpaqueComplexFallbackReturnShouldStayAny(
	...
	if opaqueIndexedFallbackRecoveryShouldStayConcrete(expression, bodyExpression, inferredReturnType, dependencyTypes) {
		return false
	}
...
func opaqueIndexedFallbackRecoveryShouldStayConcrete(expression, bodyExpression, inferredReturnType string, dependencyTypes []string) bool {
	if !parityModeEnabled() {
		return false
	}

	inferredReturnType = normalizeInferredTypeText(strings.TrimSpace(inferredReturnType))
	if inferredReturnType == "" || isAnyLikeType(inferredReturnType) || isLooselyTypedType(inferredReturnType) {
		return false
	}
	if isPrimitiveLikeUnionType(inferredReturnType) ||
		isBroadPrimitiveSelectorType(inferredReturnType) ||
	...
	left, ok := selectorSimpleFallbackAccessExpression(bodyExpression)
	if !ok {
		return false
	}
	...
	root, path, ok := parseSourceAccessPath(left)
	...
	if rootIndex == -1 || rootIndex >= len(dependencyTypes) {
		return false
	}

	rootType := normalizeSourceTypeText(strings.TrimSpace(dependencyTypes[rootIndex]))
	if rootType == "" || isAnyLikeType(rootType) || isLooselyTypedType(rootType) {
		return false
	}

	for _, wrapper := range []string{"Readonly", "Partial", "DeepPartial"} {
		if inner, ok := parseSingleGenericTypeArgument(rootType, wrapper); ok {
			rootType = normalizeSourceTypeText(strings.TrimSpace(inner))
		}
	}

	containerValueType := ""
	switch {
	case strings.HasPrefix(rootType, "Record<"):
		_, valueType, ok := parseRecordTypeArguments(rootType)
		if !ok {
			return false
		}
		containerValueType = normalizeSourceTypeText(strings.TrimSpace(valueType))
	case strings.HasSuffix(rootType, "[]") || strings.HasPrefix(rootType, "Array<") || strings.HasPrefix(rootType, "ReadonlyArray<"):
		elementType, ok := parseNormalizedArrayElementType(rootType)
		if !ok {
			return false
		}
		containerValueType = normalizeSourceTypeText(strings.TrimSpace(elementType))
	default:
		return false
	}
	if containerValueType == "" || isAnyLikeType(containerValueType) || isLooselyTypedType(containerValueType) {
		return false
	}
	...
	for _, segment := range path {
		...
		for index, name := range info.ParameterNames {
			if strings.TrimSpace(name) != strings.TrimSpace(segment.Text) || index >= len(dependencyTypes) {
				continue
			}
			indexType := normalizeSourceTypeText(strings.TrimSpace(dependencyTypes[index]))
			if indexType == "" || isAnyLikeType(indexType) || isLooselyTypedType(indexType) {
				return true
			}
		}
	}
...
func internalHelperDirectFallbackConcreteDependencyReturnType(expression, bodyExpression string, dependencyTypes []string) string {
	...
	concreteReturnType := ""
	sawDeferredCompositeFallback := false
	sawNullishFallback := findLastTopLevelOperator(bodyExpression, "??") != -1
	for _, operand := range operands {
		operand = strings.TrimSpace(unwrapWrappedExpression(operand))
		if operand == "" {
			return ""
		}
		if sourceFallbackExpressionShouldDefer(operand) {
			if operand == "[]" || operand == "{}" {
				sawDeferredCompositeFallback = true
			}
			continue
		}
		parameterIndex := -1
	...
	if sawNullishFallback && sawDeferredCompositeFallback {
		if stripped := typeTextWithoutNullish(concreteReturnType); stripped != "" {
			concreteReturnType = stripped
		}
	}
...
func sourceParameterTypeHintsWithContext(source, file, parameters string, state *buildState) map[string]string {
	...
	for _, parameter := range parsed {
		...
		if destructuredText := sourceParameterDestructuringText(parameter.Text); destructuredText != "" {
			for name, hintedType := range sourceDestructuredParameterTypeHints(source, file, destructuredText, typeText, state) {
				hints[name] = hintedType
			}
		}
	}
	return hints
...
func actionParameterTypeHintShouldUseExpandedType(rawType, expandedType string) bool {
	...
	if genericSpecializesActionPayloadType(rawType, expandedType) {
		return true
	}
	if expandedType != "" &&
...
func expandSourceParameterHintTypeText(source, file, typeText string, state *buildState) string {
	...
	if specialized := specializeCompoundTypeTextWithDefaultArguments(source, file, typeText, state); specialized != "" {
		return specialized
	}
	if indexedExpanded {
		return typeText
...
func specializeDefaultedGenericReferencesInTypeText(source, file, typeText string, state *buildState) string {
	text := normalizeSourceTypeText(strings.TrimSpace(typeText))
	if text == "" {
		return ""
	}
	...
	for index := 0; index < len(text); {
		...
		case '\'':
			...
			builder.WriteString(text[index : end+1])
			index = end + 1
			continue
		case '"':
			end, err := skipQuoted(text, index, '"')
		...
		previous := previousNonWhitespaceByte(text, start)
		next := nextNonWhitespaceByte(text, index)
		if previous == '.' || next == '.' || next == '<' || next == ':' || (next == '?' && nextNonWhitespaceByte(text, index+1) == ':') || !shouldImportIdentifier(identifier) {
			builder.WriteString(identifier)
			continue
		}
		specialized := defaultedGenericInstantiationForIdentifier(source, file, identifier, state, cache)
		if specialized == "" || specialized == identifier {
			builder.WriteString(identifier)
			continue
		}
		builder.WriteString(specialized)
		changed = true
	}
	if !changed {
		return ""
	}
	return normalizeSourceTypeText(builder.String())
}

func defaultedGenericInstantiationForIdentifier(source, file, identifier string, state *buildState, cache map[string]string) string {
	if cached, ok := cache[identifier]; ok {
		return cached
	}
	cache[identifier] = ""
	if specialized := instantiatedTypeTextWithDefaultArguments(source, identifier); specialized != "" {
		cache[identifier] = specialized
		return specialized
	}
	if file == "" {
		return ""
	...
	specialized := instantiatedTypeTextWithDefaultArguments(string(content), candidate.ImportedName)
	if specialized == "" {
		return ""
	}
	cache[identifier] = specialized
...
func indexedAccessPropertyType(source, file, baseType, propertyName string, state *buildState) string {
	baseTypeText, baseSource, ok := resolveIndexedAccessBaseType(source, file, baseType, state)
	if !ok || baseTypeText == "" {
		return ""
	}
	...
	if collapsed := collapseQualifiedEnumMemberUnionToOwnerAlias(baseSource, propertyType); collapsed != "" {
		return collapsed
	}
	return propertyType
}

func resolveIndexedAccessBaseType(source, file, baseType string, state *buildState) (string, string, bool) {
	...
	expanded := expandLocalSourceTypeText(string(content), candidate.ImportedName)
	if expanded == "" {
		return "", "", false
	}
	return expanded, string(content), true
}
...
func specializeCompoundTypeTextWithDefaultArguments(source, file, typeText string, state *buildState) string {
	text := normalizeSourceTypeText(strings.TrimSpace(typeText))
	if text == "" {
		return ""
	}
	...
	specialized := make([]string, 0, len(parts))
	changed := false
	for _, part := range parts {
		part = normalizeSourceTypeText(strings.TrimSpace(part))
		if part == "" {
		...
		next := part
		if current := specializeLocalTypeTextWithDefaultArguments(source, part); current != "" {
			next = normalizeSourceTypeText(current)
		} else if current := specializeImportedTypeTextWithDefaultArguments(source, file, part, state); current != "" {
			next = normalizeSourceTypeText(current)
		}
		if next != part {
			changed = true
	...
	if !changed || len(specialized) != len(parts) {
		return ""
	}
	return normalizeSourceTypeText(strings.Join(specialized, " | "))
}
...
func collapseQualifiedEnumMemberUnionToOwnerAlias(source, typeText string) string {
	text := normalizeSourceTypeText(strings.TrimSpace(typeText))
	if text == "" {
		return ""
	}
	...
	for _, part := range parts {
		...
		currentOwner := strings.TrimSpace(part[:index])
		currentMember := strings.TrimSpace(part[index+1:])
	...
	if mainCount <= 1 || owner == "" || !sourceHasLocalEnumDeclaration(source, owner) {
		return ""
	}

	resultParts := []string{owner}
	resultParts = append(resultParts, nullish...)
	return normalizeSourceTypeText(strings.Join(resultParts, " | "))
}

func sourceHasLocalEnumDeclaration(source, name string) bool {
	name = strings.TrimSpace(name)
	if name == "" {
		return false
	}
	pattern := regexp.MustCompile(`(?m)^\s*(?:export\s+)?(?:declare\s+)?enum\s+` + regexp.QuoteMeta(name) + `\b`)
...
func sourceDestructuredParameterTypeHints(source, file, parameterText, defaultType string, state *buildState) map[string]string {
	...
	bindings := sourceDestructuredParameterBindings(text)
	if len(bindings) == 0 {
		return nil
	}
	hints := map[string]string{}
	for name, sourceName := range bindings {
...
func sourceDestructuredParameterBindings(parameterText string) map[string]string {
	text := strings.TrimSpace(parameterText)
	if text == "" || text[0] != '{' {
		return nil
	}
...
func sourceObjectLiteralTypeTextWithHintsOptions(
	...
	for _, segment := range segments {
		...
		if strings.HasPrefix(text, "...") {
			...
			spreadMembers, _, ok := spreadObjectTypeMembersWithOptionalFlags(spreadType)
			if !ok {
				continue
			}
			for name, value := range spreadMembers {
...
func sourceObjectLiteralTypeTextWithContextOptions(
	...
	for _, segment := range segments {
		...
		if strings.HasPrefix(text, "...") {
			...
			if expanded := expandImportedTypeAliasTextWithContext(source, file, spreadType, state); expanded != "" {
				spreadType = normalizeSourceTypeText(expanded)
			}
			spreadMembers, spreadOptionals, ok := spreadObjectTypeMembersWithOptionalFlags(spreadType)
			if !ok {
				continue
			}
			for name, value := range spreadMembers {
...
func spreadObjectTypeMembersWithOptionalFlags(typeText string) (map[string]string, map[string]bool, bool) {
	if members, optionals, ok := parseObjectTypeMembersWithOptionalFlags(typeText, true); ok {
		return members, optionals, true
	}
	keyType, valueType, ok := parseRecordTypeArguments(typeText)
	if !ok {
		return nil, nil, false
	}
	keyType = normalizeSourceTypeText(strings.TrimSpace(keyType))
	valueType = normalizeSourceTypeText(strings.TrimSpace(valueType))
	if keyType == "" || valueType == "" {
		return nil, nil, false
	}
	return map[string]string{
			fmt.Sprintf("[x: %s]", keyType): valueType,
		},
		map[string]bool{
			fmt.Sprintf("[x: %s]", keyType): false,
		},
		true
}
...
func sourceExpressionTypeTextWithHints(source, expression string, hints map[string]string) string {
	...
	if unaryBooleanType := sourceUnaryBooleanExpressionTypeText(text); unaryBooleanType != "" {
		return unaryBooleanType
	}
	if asserted := sourceAssertedType(text); asserted != "" {
		return normalizeSourceTypeText(asserted)
	}
...
func sourceExpressionTypeTextWithContext(source, file, expression string, hints map[string]string, state *buildState) string {
	...
	if indexedType := sourceIndexedAccessExpressionTypeTextWithContext(source, file, text, hints, state); indexedType != "" {
		return indexedType
	}
...
func sourceUnaryBooleanExpressionTypeText(expression string) string {
	text := strings.TrimSpace(unwrapWrappedExpression(expression))
	if text == "" {
		return ""
	}
	if sourceUnaryBooleanExpressionHasTrailingOperators(text) {
		return ""
	}
	if strings.HasPrefix(text, "!!") {
...
func sourceUnaryBooleanExpressionHasTrailingOperators(expression string) bool {
	if _, _, ok := findTopLevelConditional(expression); ok {
		return true
	}
	for _, operator := range []string{"&&", "||", "??", "!==", "===", "!=", "==", ">=", "<=", ">", "<", "+", "-", "*", "/", "%"} {
		if findLastTopLevelOperator(expression, operator) != -1 {
			return true
		}
	}
	return false
...
func sourceIndexedAccessExpressionTypeTextWithContext(source, file, expression string, hints map[string]string, state *buildState) string {
	text := strings.TrimSpace(unwrapWrappedExpression(expression))
	if len(text) < 3 || text[len(text)-1] != ']' {
		return ""
	}
	bracketIndex := strings.LastIndex(text, "[")
	if bracketIndex <= 0 || bracketIndex >= len(text)-1 {
		return ""
	}
	baseExpression := strings.TrimSpace(text[:bracketIndex])
	indexExpression := strings.TrimSpace(text[bracketIndex+1 : len(text)-1])
	if baseExpression == "" || indexExpression == "" {
		return ""
	}
	if !isNumericLiteralType(indexExpression) &&
		!sourceExpressionTypeLooksNumeric(sourceExpressionTypeTextWithContext(source, file, indexExpression, hints, state)) {
		return ""
	}

	baseType := normalizeInferredTypeText(sourceExpressionTypeTextWithContext(source, file, baseExpression, hints, state))
	if baseType == "" {
		return ""
	}
	baseType = normalizeInternalHelperParameterType(baseType)
	elementType, ok := parseNormalizedArrayElementType(baseType)
	if !ok || elementType == "" {
		return ""
	}
	return elementType
...
func sourceBuiltInCallExpressionTypeTextWithContext(source, file, expression, callee string, hints map[string]string, state *buildState) string {
	...
	case "Object.entries":
		...
		collectionType := normalizeSourceTypeText(sourceExpressionTypeTextWithContext(source, file, strings.TrimSpace(arguments[0]), hints, state))
		if collectionType == "" {
			return "any[]"
		}
		if _, valueType, ok := parseRecordTypeArguments(collectionType); ok {
			return arrayTypeText("[string, " + valueType + "]")
		}
		if members, ok := parseObjectTypeMembersWithOptionalUndefined(collectionType, true); ok && len(members) > 0 {
			valueType := ""
			for _, memberType := range members {
				memberType = normalizeSourceTypeText(strings.TrimSpace(memberType))
				if memberType == "" {
					continue
				}
				if valueType == "" {
				...
				valueType = mergeNormalizedTypeUnion(valueType, memberType)
			}
			if valueType != "" {
				return arrayTypeText("[string, " + valueType + "]")
			}
...
func sourceFallbackExpressionShouldDefer(expression string) bool {
	text := strings.TrimSpace(unwrapWrappedExpression(expression))
	if text == "" {
		return false
	}
	...
	return sourceFallbackLiteralShouldDefer(sourceLiteralTypeText(text))
}
...
func sourceExpressionTypeFromTypeProbeRange(source, file string, valueStart, valueEnd int, state *buildState) string {
	...
	fallback := ""
	if source != "" {
		expression := trimLeadingSourceTrivia(source[valueStart:valueEnd])
		expression = trimLeadingSourceTrivia(unwrapWrappedExpression(expression))
		if identifier, ok := sourceIdentifierExpression(expression); ok && identifier != "" {
			position := skipTrivia(source, valueStart)
			typeText := normalizeSourceTypeText(state.cachedTypeOfSymbolAtPositionString(projectID, file, position))
			if typeText != "" && !isFunctionLikeTypeText(typeText) {
				if !typeTextNeedsSourceRecovery(typeText) {
					return typeText
				}
				fallback = typeText
			}
	...
	if element := sourceSelectorProjectorElement(expression); element != "" {
		if info, ok := parseSourceArrowInfo(element); ok {
			if returnType := sourceReturnExpressionTypeWithContext(source, file, info.Body, info.BlockBody, info.ParameterNames, dependencyTypes, info.Async, state); returnType != "" {
				...
				if concreteReturn := internalHelperDirectFallbackConcreteDependencyReturnType(expression, bodyExpression, dependencyTypes); concreteReturn != "" &&
					(isAnyLikeType(returnType) || isLooselyTypedType(returnType)) {
...
func typeTextWithoutNullish(typeText string) string {
	text := normalizeSourceTypeText(strings.TrimSpace(typeText))
	if text == "" {
		return ""
	}
...
func sourceParameterDestructuringText(text string) string {
	text = strings.TrimSpace(text)
	text = strings.TrimPrefix(text, "...")
	baseText, _ := stripTopLevelParameterDefault(text)
	...
	end, err := findMatching(baseText, 0, open, close)
	if err != nil || end < 0 {
		return ""
	}
	remainder := strings.TrimSpace(baseText[end+1:])
	if remainder != "" && !strings.HasPrefix(remainder, ":") {
		return ""
	}
	return strings.TrimSpace(baseText[:end+1])
}

func sourceParameterIsUntypedDestructured(text string) bool {
	bindingText := sourceParameterDestructuringText(text)
	if bindingText == "" {
		return false
	}
	...
	baseText, _ := stripTopLevelParameterDefault(text)
	baseText = strings.TrimSpace(baseText)
	return strings.TrimSpace(strings.TrimPrefix(baseText[len(bindingText):], "?")) == ""
}
...
func normalizeSourceTypeTextWithOptions(text string, collapseOptionalUndefined bool) string {
	...
	text = strings.Join(strings.Fields(text), " ")
	text = stripImportTypeQualifiers(text)
	text = strings.ReplaceAll(text, "( ", "(")
	text = strings.ReplaceAll(text, " )", ")")
	text = strings.ReplaceAll(text, ",)", ")")
	...
	appendListener := func(name string, member *MemberReport, allowGenericFallback bool) {
		...
		if member != nil {
			if payload := sharedListenerPayloadTypeFromMember(*member); payload != "" && !isAnyLikeType(payload) {
				payloadType = payload
				actionType = normalizeSourceTypeTextWithOptions(fmt.Sprintf("{ type: string; payload: %s }", payload), false)
...
func sharedListenerPayloadTypeFromMember(member MemberReport) string {
	parameters, _, ok := splitFunctionType(preferredMemberFunctionTypeTextPreservingOptionalUndefined(member))
	if !ok {
		return ""
	}
	return normalizeSourceTypeTextWithOptions(firstParameterType(parameters), false)
}
...
func refineActionPayloadTypeFromFunction(functionType, payloadType string) string {
	...
	changed := false
	for _, parameter := range parsedParameters {
		normalizedPart := normalizeParameterDeclarationText(parameter.Text)
		if normalizedPart == "" {
			continue
		}
		...
		candidateType := normalizeSourceTypeText(typeText)
		if strings.HasSuffix(strings.TrimSpace(rawName), "?") && !typeTextContainsUnquotedStandaloneToken(candidateType, "undefined") {
			candidateType = normalizeSourceTypeText(candidateType + " | undefined")
		}
		if candidateType == "" || strings.Contains(candidateType, "...") {
			continue
		}
		member.Type = candidateType
		payloadMembers[name] = member
		changed = true
	}
	if !changed {
		return payloadType
	}
	return renderActionPayloadObjectMembers(payloadMembers)
}
...
func preserveUntypedDestructuredActionParameters(functionType, sourceParameters string) string {
	functionType = strings.TrimSpace(functionType)
	sourceParameters = strings.TrimSpace(sourceParameters)
	if functionType == "" || sourceParameters == "" {
		return functionType
	}

	currentParameters, returnType, ok := splitFunctionType(functionType)
	if !ok {
		return functionType
	}

	sourceParts, ok := splitFunctionParameterParts(sourceParameters)
	if !ok || len(sourceParts) == 0 {
		return functionType
	}
	currentParts, ok := splitFunctionParameterParts(currentParameters)
	if !ok || len(currentParts) != len(sourceParts) {
		return functionType
	}

	refined := make([]string, 0, len(currentParts))
	changed := false
	for index, currentPart := range currentParts {
		normalizedCurrent := normalizeParameterDeclarationText(currentPart)
		if normalizedCurrent == "" {
			continue
		}
		if !sourceParameterIsUntypedDestructured(sourceParts[index]) {
			refined = append(refined, normalizedCurrent)
			continue
		}
		rawName, _, ok := splitTopLevelPropertyRaw(normalizedCurrent)
		if !ok || sourceParameterDestructuringText(rawName) == "" {
			refined = append(refined, normalizedCurrent)
			continue
		}
		refined = append(refined, strings.TrimSpace(rawName)+": any")
		changed = true
	}
	if !changed {
		return functionType
	}
	return "(" + strings.Join(refined, ", ") + ") => " + returnType
}
...
func (s *buildState) close() {
	...
	s.typeTextByPos = nil
	s.typeTextBySymbolPos = nil
	s.typeTextByLocation = nil
	s.callbackByPos = nil
	s.signatureByPos = nil
	s.exportedTypeNamesByFile = nil
	s.packageExportCandidatesByPath = nil
	s.importedValueSourcesByFile = nil
	s.transitiveTypeImportsByRef = nil
...
func (s *buildState) cachedTypeOfSymbolAtPositionString(projectID, file string, position int) string {
	if s == nil || s.apiClient == nil || projectID == "" || file == "" || position <= 0 {
		return ""
	}
	position = s.normalizedPositionForFile(file, position)
	if s.typeTextBySymbolPos == nil {
		s.typeTextBySymbolPos = map[string]string{}
	}
	key := s.positionKey(projectID, file, position)
	if cached, ok := s.typeTextBySymbolPos[key]; ok {
		return cached
	}

	symbol, err := s.apiClient.GetSymbolAtPosition(
		tsgoapi.WithTimeout(context.Background(), s.timeout),
		s.apiSnapshot,
		projectID,
		file,
	...
	if err != nil || symbol == nil || symbol.ID == "" {
		s.typeTextBySymbolPos[key] = ""
		return ""
	}

	typeID := ""
	declared, err := s.apiClient.GetDeclaredTypeOfSymbol(
		tsgoapi.WithTimeout(context.Background(), s.timeout),
		s.apiSnapshot,
		projectID,
		symbol.ID,
	)
	if err == nil && declared != nil && declared.ID != "" {
		typeID = declared.ID
	}
	if typeID == "" {
	...
	if typeID == "" {
		typeID = s.firstTypeOfSymbol(projectID, symbol.ID)
	}
	if typeID == "" {
		s.typeTextBySymbolPos[key] = ""
		return ""
	...
	text := s.cachedTypeString(projectID, typeID)
	s.typeTextBySymbolPos[key] = text
	return text
}
...
func (s *buildState) loadParsedFile(file string) (parsedFileCache, error) {
	...
	file = filepath.Clean(file)
	loadStart := time.Now()
	defer maybeLogBuildTiming(filepath.Base(file), "", "loadParsedFile", loadStart)
	if s.parsedFileByFile != nil {
		if entry, ok := s.parsedFileByFile[file]; ok {
			return entry, nil
		}
	...
	for _, reference := range references {
		...
		case "actions":
			...
			for _, name := range reference.Names {
				...
				if action, actionImports, ok := synthesizeConnectedActionFromSymbols(name, actionMembers, actionCreatorMembers); ok {
					...
					imports = mergeTypeImports(imports, actionImports)
		...
		case "values", "props":
			...
			for _, name := range reference.Names {
				...
				if member, found := findMemberReport(valueMembers, name.SourceName); found && strings.TrimSpace(member.TypeString) != "" {
					...
					imports = mergeTypeImports(
						imports,
						collectExplicitImportTypeImports(memberExplicitImportTypeTexts(member)),
						memberCollectedTypeImports(member),
					)
...
func resolveConnectedSectionMembersBySymbol(
	...
	for _, position := range connectedTargetProbePositions(source, reference) {
		...
		members, ok := connectedTypeMembers(state.apiClient, state.apiSnapshot, projectID, state.timeout, sectionType.ID, file)
		if ok {
			return members, true
		}
	}
...
func connectedTypeMembers(
	...
	for _, property := range properties {
		...
		if err == nil && propertyType != nil {
			...
			member.TypeImports = mergeTypeImports(member.TypeImports, typeImportsForTypeSymbol(client, snapshot, projectID, timeout, propertyType.ID, contextFile))
			...
			if err == nil && len(signatures) > 0 {
				...
				member.TypeImports = mergeTypeImports(member.TypeImports, signatureParameterTypeImports(client, snapshot, projectID, timeout, signatures[0], contextFile))
				...
				if err == nil && returnType != nil {
					...
					member.TypeImports = mergeTypeImports(member.TypeImports, typeImportsForTypeSymbol(client, snapshot, projectID, timeout, returnType.ID, contextFile))
...
func signatureParameterTypeImports(
	client *tsgoapi.Client,
	snapshot string,
	projectID string,
	timeout time.Duration,
	signature *tsgoapi.SignatureResponse,
	contextFile string,
) []TypeImport {
	...
	var imports []TypeImport
	for _, symbolID := range signature.Parameters {
		imports = mergeTypeImports(imports, typeImportsForSymbolType(client, snapshot, projectID, timeout, symbolID, contextFile))
	}
	return imports
}

func typeImportsForSymbolType(
	client *tsgoapi.Client,
	snapshot string,
	projectID string,
	timeout time.Duration,
	...
	if client == nil || projectID == "" || symbolID == "" {
		return nil
	}
	if declaredType, err := client.GetDeclaredTypeOfSymbol(tsgoapi.WithTimeout(context.Background(), timeout), snapshot, projectID, symbolID); err == nil && declaredType != nil {
		if imports := typeImportsForTypeSymbol(client, snapshot, projectID, timeout, declaredType.ID, contextFile); len(imports) > 0 {
			return imports
		}
	}
	if symbolType, err := client.GetTypeOfSymbol(tsgoapi.WithTimeout(context.Background(), timeout), snapshot, projectID, symbolID); err == nil && symbolType != nil {
		return typeImportsForTypeSymbol(client, snapshot, projectID, timeout, symbolType.ID, contextFile)
	}
	return nil
}

func typeImportsForTypeSymbol(
	client *tsgoapi.Client,
	snapshot string,
	projectID string,
	timeout time.Duration,
	...
	if client == nil || projectID == "" || typeID == "" {
		return nil
	}
	symbol, err := client.GetSymbolOfType(tsgoapi.WithTimeout(context.Background(), timeout), snapshot, projectID, typeID)
	if err != nil {
		return nil
	}
	return typeImportsFromSymbol(symbol, contextFile)
}

func typeImportsFromSymbol(symbol *tsgoapi.SymbolResponse, contextFile string) []TypeImport {
	if symbol == nil {
		return nil
	}
	...
	importPath := bestImportableDeclarationPath(symbol, contextFile)
	if importPath == "" {
		return nil
	}
	return []TypeImport{{Path: importPath, Names: []string{name}}}
}

func bestImportableDeclarationPath(symbol *tsgoapi.SymbolResponse, contextFile string) string {
	if symbol == nil {
		return ""
	}
	seen := map[string]bool{}
	paths := make([]string, 0, len(symbol.Declarations)+1)
	...
	for _, declaration := range symbol.Declarations {
		if path, ok := importableDeclarationPath(declaration, contextFile); ok {
			appendPath(path)
		}
	}
	if path, ok := importableDeclarationPath(symbol.ValueDeclaration, contextFile); ok {
		appendPath(path)
	}
	if len(paths) == 0 {
		return ""
	...
	sort.Slice(paths, func(i, j int) bool {
		return declarationImportPathLess(paths[i], paths[j])
	})
	return paths[0]
}

func importableDeclarationPath(location, contextFile string) (string, bool) {
	path, ok := declarationFilePath(location)
	if !ok {
		return "", false
	}
	path = filepath.Clean(path)
	if contextFile != "" && filepath.Clean(contextFile) == path {
		return "", false
	}
	if !isImportableDeclarationFile(path) {
		return "", false
	}
	return path, true
}

func declarationFilePath(location string) (string, bool) {
...
func isImportableDeclarationFile(path string) bool {
	path = filepath.Clean(strings.TrimSpace(path))
	if path == "" {
		return false
	}
	...
	if strings.Contains(slashPath, "/typescript/lib/") {
		return false
	}
...
func declarationImportPathLess(left, right string) bool {
	left = filepath.Clean(left)
	right = filepath.Clean(right)

	leftNodeModules := strings.Contains(filepath.ToSlash(left), "/node_modules/")
	rightNodeModules := strings.Contains(filepath.ToSlash(right), "/node_modules/")
	if leftNodeModules != rightNodeModules {
		return leftNodeModules
	}

	leftIndex := strings.HasPrefix(filepath.Base(left), "index.")
	rightIndex := strings.HasPrefix(filepath.Base(right), "index.")
...
func synthesizeConnectedActionFromSymbols(name connectedName, actionMembers, actionCreatorMembers []MemberReport) (ParsedAction, []TypeImport, bool) {
	...
	return ParsedAction{
			...
			collectExplicitImportTypeImports(memberExplicitImportTypeTexts(actionMember, creatorMember)),
			memberCollectedTypeImports(actionMember, creatorMember),
		), true
}
...
func parseReducerStateType(typeText string) (string, bool) {
	...
	if len(parts) > 1 {
		handlersText := strings.TrimSpace(parts[1])
		if recovered := recoverReducerStateTypeFromHandlers(stateType, handlersText); recovered != "" {
			stateType = recovered
		}
		if widened := widenReducerStateTypeFromHandlers(stateType, handlersText); widened != "" {
			stateType = widened
		}
	}
...
func parseCollapsedReducerTupleStateType(typeText string) (string, bool) {
	...
	if recovered := recoverReducerStateTypeFromHandlers(stateType, handlerType); recovered != "" {
		stateType = recovered
	}
...
func selectorSourceHasExplicitAnyInputState(expression string) bool {
	inputFunction := firstTopLevelArrayElement(strings.TrimSpace(expression))
	if inputFunction == "" {
		return false
	}
	...
	params, ok := parseFunctionParameters(info.Parameters)
	if !ok || len(params) != 1 {
		return false
	}
	return normalizeInferredTypeText(strings.TrimSpace(params[0].Type)) == "any"
}

func selectorSourceHasRecoverableTypedUnnamedDependency(expression string) bool {
	dependencyNames := sourceSelectorDependencyNamesWithPlaceholders(expression)
	if len(dependencyNames) == 0 {
		return false
	}
...
func parseObjectTypeMembersWithOptionalUndefined(typeText string, includeOptionalUndefined bool) (map[string]string, bool) {
	properties, _, ok := parseObjectTypeMembersWithOptionalFlags(typeText, includeOptionalUndefined)
	return properties, ok
}

func parseObjectTypeMembersWithOptionalFlags(typeText string, includeOptionalUndefined bool) (map[string]string, map[string]bool, bool) {
	text := strings.TrimSpace(typeText)
	if !strings.HasPrefix(text, "{") || !strings.HasSuffix(text, "}") {
		return nil, nil, false
	}
	...
	body := strings.TrimSpace(text[1 : len(text)-1])
	if body == "" {
		return map[string]string{}, map[string]bool{}, true
	}
	...
	optionalProperties := make(map[string]bool, len(entries))
	for _, entry := range entries {
		rawName, value, ok := splitTopLevelPropertyRaw(entry)
		if !ok {
			continue
		...
		name = strings.TrimSuffix(name, "?")
		name = strings.Trim(name, `"'`)
		value = normalizeSourceTypeText(trimLeadingTypeMemberContinuation(value))
		if name == "" || value == "" {
			continue
		}
		...
		properties[name] = value
		optionalProperties[name] = optional
	}
	return properties, optionalProperties, true
}

func trimLeadingTypeMemberContinuation(value string) string {
...
func splitTopLevelTypeMembers(source string) ([]string, error) {
	...
	for i := 0; i < len(source); i++ {
		...
		case '\n':
			if parenDepth == 0 && bracketDepth == 0 && braceDepth == 0 && angleDepth == 0 {
				if typeMemberContinuesAcrossNewline(source, start, i) {
					continue
				}
				member := strings.TrimSpace(source[start:i])
				if member != "" {
...
func typeMemberContinuesAcrossNewline(source string, start, newlineIndex int) bool {
	current := strings.TrimSpace(source[start:newlineIndex])
	if current == "" {
		return true
	}
...
func recoverReducerStateTypeFromHandlers(stateType, handlersText string) string {
	stateType = normalizeInferredTypeText(strings.TrimSpace(stateType))
	if stateType != "" && !isAnyLikeType(stateType) && !isLooselyTypedType(stateType) {
		return ""
	}
	...
	recovered := ""
	for _, propertyType := range properties {
		parameters, _, ok := splitFunctionType(propertyType)
		if !ok {
			continue
		}
		params, ok := parseFunctionParameters(parameters)
		...
		candidate := normalizeInferredTypeText(strings.TrimSpace(params[0].Type))
		if candidate == "" || isAnyLikeType(candidate) || isLooselyTypedType(candidate) {
			continue
		}
		if recovered == "" {
			recovered = candidate
		...
		recovered = mergeNormalizedTypeUnion(recovered, candidate)
	}

	return normalizeInferredTypeText(recovered)
}
...
func stripImportTypeQualifiers(typeText string) string {
	text := strings.TrimSpace(typeText)
	if text == "" {
		return ""
	}
	return importTypeReferencePattern.ReplaceAllString(text, "${3}")
}

func memberExplicitImportTypeTexts(members ...MemberReport) []string {
	texts := make([]string, 0, len(members)*4)
	...
	for _, member := range members {
		appendText(preferredPrintedOrRawTypeText(member.PrintedTypeNode, member.TypeString))
		appendText(preferredPrintedOrRawTypeText(member.PrintedReturnTypeNode, member.ReturnTypeString))
		appendText(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedTypeNode, member.ProjectorDirectTypeString))
		appendText(preferredPrintedOrRawTypeText(member.ProjectorDirectPrintedReturnTypeNode, member.ProjectorDirectReturnTypeString))
	}
	return texts
}

func memberCollectedTypeImports(members ...MemberReport) []TypeImport {
	imports := make([][]TypeImport, 0, len(members))
	for _, member := range members {
		if len(member.TypeImports) > 0 {
			imports = append(imports, member.TypeImports)
		}
	}
	return mergeTypeImports(imports...)
}
...
type importedSourceFile struct {
	File   string
	Source string
}

type cachedTypeImportsResult struct {
	Imports []TypeImport
	OK      bool
}
...
func mergeTypeImports(groups ...[]TypeImport) []TypeImport {
	...
	return typeImportsFromGroupedNames(grouped)
}

func typeImportsFromGroupedNames(grouped map[string]map[string]bool) []TypeImport {
...
func preferSourceTypeImportsForUsedReferences(source string, typeTexts []string, imports []TypeImport) []TypeImport {
	if source == "" || len(imports) == 0 || len(typeTexts) == 0 {
		return imports
	}

	directImports := map[string]importCandidate{}
	for alias, candidate := range parseNamedImports(source) {
		directImports[alias] = candidate
	}
	for alias, candidate := range parseDefaultImports(source) {
		directImports[alias] = candidate
	}
	...
	preferredByReference := map[string]importCandidate{}
	for reference, candidate := range directImports {
		if !used[reference] || candidate.Path == "" || candidate.Name == "" || parityModeShouldOmitCollectedImportPath(candidate.Path) {
			continue
		}
		preferredByReference[reference] = candidate
	}
	if len(preferredByReference) == 0 {
		return imports
	}
	...
	add := func(path, name string) {
		...
		if grouped[path] == nil {
			grouped[path] = map[string]bool{}
		}
		grouped[path][name] = true
	}

	for _, item := range imports {
		for _, name := range item.Names {
			reference := importReferenceName(name)
			if preferred, ok := preferredByReference[reference]; ok &&
				(preferred.Path != item.Path || preferred.Name != name) {
				continue
			}
			add(item.Path, name)
		}
	}
	for _, preferred := range preferredByReference {
		add(preferred.Path, preferred.Name)
	}

	return mergeTypeImports(typeImportsFromGroupedNames(grouped))
}

func dropRedundantBarePackageExplicitImportTypeImports(imports []TypeImport, typeTexts []string) []TypeImport {
	if len(imports) == 0 || len(typeTexts) == 0 {
		return imports
	}

	explicitBareImports := collectExplicitBarePackageImportTypeReferences(typeTexts)
	if len(explicitBareImports) == 0 {
		return imports
	}

	pathsByReference := map[string]map[string]bool{}
	for _, item := range imports {
		for _, name := range item.Names {
			reference := importReferenceName(name)
			if reference == "" {
			...
			if pathsByReference[reference] == nil {
				pathsByReference[reference] = map[string]bool{}
			}
			pathsByReference[reference][item.Path] = true
		}
	}
	...
	for _, item := range imports {
		for _, name := range item.Names {
			reference := importReferenceName(name)
			if explicitBareImports[item.Path][reference] && importReferenceHasAnotherPath(pathsByReference[reference], item.Path) {
				continue
			}
			if grouped[item.Path] == nil {
				grouped[item.Path] = map[string]bool{}
...
func collectExplicitBarePackageImportTypeReferences(typeTexts []string) map[string]map[string]bool {
	grouped := map[string]map[string]bool{}
	for _, typeText := range typeTexts {
		for _, match := range importTypeReferencePattern.FindAllStringSubmatch(typeText, -1) {
			if len(match) != 4 {
				continue
			}
			importPath := strings.TrimSpace(firstNonEmpty(match[1], match[2]))
			name := strings.TrimSpace(match[3])
			if importPath == "" || name == "" || strings.HasPrefix(importPath, ".") || strings.HasPrefix(importPath, "/") {
				continue
			}
			if grouped[importPath] == nil {
				grouped[importPath] = map[string]bool{}
			}
			grouped[importPath][name] = true
		}
	}
	return grouped
}

func importReferenceHasAnotherPath(paths map[string]bool, currentPath string) bool {
...
func collectTypeImportsForTypeTexts(source, file string, typeTexts []string, state *buildState, coveredIdentifiers map[string]bool) []TypeImport {
	return collectTypeImportsForTypeTextsRecursive(source, file, typeTexts, state, coveredIdentifiers, map[string]bool{})
}

func collectTypeImportsForTypeTextsRecursive(source, file string, typeTexts []string, state *buildState, coveredIdentifiers map[string]bool, visitedFiles map[string]bool) []TypeImport {
	started := time.Now()
	if len(typeTexts) == 0 {
		return nil
	}
	...
	if file != "" {
		...
		defer delete(visitedFiles, file)
	}
	defer maybeLogBuildTiming(filepath.Base(file), "", fmt.Sprintf("collectTypeImports[%d]", len(typeTexts)), started)
	...
	packageExportCache := map[string]map[string]importCandidate{}
	if state != nil {
		if state.exportedTypeNamesByFile == nil {
			state.exportedTypeNamesByFile = map[string]map[string]bool{}
		}
		if state.packageExportCandidatesByPath == nil {
			state.packageExportCandidatesByPath = map[string]map[string]importCandidate{}
		}
		exportCache = state.exportedTypeNamesByFile
		packageExportCache = state.packageExportCandidatesByPath
	}
	usedReferences := collectUsedTypeReferences(typeTexts)
	valueImportSources := collectImportedValueSourceFiles(source, file, state)
	transitiveFallbackEligible := map[string]bool{}
	...
	grouped := map[string]map[string]bool{}
	for _, item := range collectExplicitImportTypeImports(typeTexts) {
		if grouped[item.Path] == nil {
			grouped[item.Path] = map[string]bool{}
		}
		for _, name := range item.Names {
	...
	for identifier := range usedReferences.BareIdentifiers {
		...
		if !ok && len(importPaths) > 0 {
			...
			maybeLogBuildTiming(filepath.Base(file), identifier, "resolveImportedExportCandidate", resolveStart)
		}
		if !ok || candidate.Path == "" || candidate.Name == "" {
			eligible, seen := transitiveFallbackEligible[identifier]
			if !seen {
				eligible = typeTextsContainTransitiveFallbackAnchor(typeTexts, identifier)
				transitiveFallbackEligible[identifier] = eligible
			}
			if eligible {
				fallbackStart := time.Now()
				for _, item := range collectTransitiveTypeImportsFromValueSources(valueImportSources, file, identifier, state, coveredIdentifiers, visitedFiles) {
					if parityModeShouldOmitCollectedImportPath(item.Path) {
						continue
					}
					if grouped[item.Path] == nil {
						grouped[item.Path] = map[string]bool{}
				...
				maybeLogBuildTiming(filepath.Base(file), identifier, "collectTransitiveTypeImportsFromValueSources", fallbackStart)
			}
			continue
		}
		if parityModeShouldOmitCollectedImportPath(candidate.Path) {
...
func typeTextsContainTransitiveFallbackAnchor(typeTexts []string, identifier string) bool {
	if identifier == "" {
		return false
	}
	for _, typeText := range typeTexts {
		references := collectUsedTypeReferences([]string{typeText})
		if !references.BareIdentifiers[identifier] {
			continue
		}
		for other := range references.BareIdentifiers {
			if other == identifier {
				continue
			}
			if !shouldImportIdentifier(other) {
				continue
			}
			return true
		}
		for other := range references.QualifiedOwners {
			if other == identifier {
				continue
			}
			if !shouldImportIdentifier(other) {
...
func collectImportedValueSourceFiles(source, file string, state *buildState) []importedSourceFile {
	if source == "" || file == "" {
		return nil
	}
	file = filepath.Clean(file)
	if state != nil && state.importedValueSourcesByFile != nil {
		if cached, ok := state.importedValueSourcesByFile[file]; ok {
			return cloneImportedSourceFiles(cached)
		}
	}

	seen := map[string]bool{}
	sources := make([]importedSourceFile, 0)
	appendSource := func(importPath string) {
		...
		importedSource, ok := readSourceFile(resolvedFile, state)
		if !ok || importedSource == "" {
			return
		}
		seen[resolvedFile] = true
		sources = append(sources, importedSourceFile{
			File:   resolvedFile,
	...
	for _, candidate := range parseNamedValueImports(source) {
		appendSource(candidate.Path)
	}
	for _, candidate := range parseDefaultValueImports(source) {
		appendSource(candidate.Path)
	}
	for _, importPath := range parseNamespaceValueImports(source) {
		appendSource(importPath)
	}
	if state != nil {
		if state.importedValueSourcesByFile == nil {
			state.importedValueSourcesByFile = map[string][]importedSourceFile{}
		}
		state.importedValueSourcesByFile[file] = cloneImportedSourceFiles(sources)
	}
	return sources
}

func collectTransitiveTypeImportsFromValueSources(sources []importedSourceFile, file, identifier string, state *buildState, coveredIdentifiers map[string]bool, visitedFiles map[string]bool) []TypeImport {
	if len(sources) == 0 || file == "" || identifier == "" {
		return nil
	}
	file = filepath.Clean(file)
	cacheKey := ""
	if state != nil {
		if state.transitiveTypeImportsByRef == nil {
			state.transitiveTypeImportsByRef = map[string]cachedTypeImportsResult{}
		}
		cacheKey = file + "\x00" + identifier
		if cached, ok := state.transitiveTypeImportsByRef[cacheKey]; ok {
			if !cached.OK {
				return nil
			}
			return cloneTypeImports(cached.Imports)
		}
	}

	matches := make([]TypeImport, 0, 1)
	seen := map[string]bool{}
	for _, sourceFile := range sources {
		imports := collectTypeImportsForTypeTextsRecursive(
			sourceFile.Source,
			sourceFile.File,
			[]string{identifier},
			state,
		...
		imports = rebaseTypeImports(filterImportsByTypeTexts(imports, []string{identifier}), sourceFile.File, file)
		for _, item := range imports {
			var names []string
			for _, name := range item.Names {
				if importReferenceName(name) != identifier {
					continue
				}
				key := item.Path + "::" + name
				if seen[key] {
			...
			if len(names) > 0 {
				matches = append(matches, TypeImport{Path: item.Path, Names: names})
			}
		}
	}

	matches = mergeTypeImports(matches)
	count := 0
	...
	if count != 1 {
		if cacheKey != "" {
			state.transitiveTypeImportsByRef[cacheKey] = cachedTypeImportsResult{OK: false}
		}
		return nil
	}
	if cacheKey != "" {
		state.transitiveTypeImportsByRef[cacheKey] = cachedTypeImportsResult{
			Imports: cloneTypeImports(matches),
			OK:      true,
		}
	}
	return matches
...
func cloneImportedSourceFiles(items []importedSourceFile) []importedSourceFile {
	if len(items) == 0 {
		return nil
	}
	cloned := make([]importedSourceFile, len(items))
	copy(cloned, items)
	return cloned
}

func cloneTypeImports(imports []TypeImport) []TypeImport {
	if len(imports) == 0 {
		return nil
	}
	cloned := make([]TypeImport, len(imports))
	for index, item := range imports {
		cloned[index] = TypeImport{
			Path:  item.Path,
...
func readSourceFile(file string, state *buildState) (string, bool) {
	if file == "" {
		return "", false
	}
	file = filepath.Clean(file)
	if state != nil && state.parsedFileByFile != nil {
		if entry, ok := state.parsedFileByFile[file]; ok && entry.Source != "" {
	...
	return string(sourceBytes), true
}
...
func collectExplicitImportTypeImports(typeTexts []string) []TypeImport {
	grouped := map[string]map[string]bool{}
	for _, typeText := range typeTexts {
		for _, match := range importTypeReferencePattern.FindAllStringSubmatch(typeText, -1) {
			if len(match) != 4 {
